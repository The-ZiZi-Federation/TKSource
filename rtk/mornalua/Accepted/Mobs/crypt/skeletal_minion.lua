...
--	mob:talk(0,""..target.name)
	...
	if mob.registry["strike_timer"] >= 16 then
		if facingPC ~= nil then
			skeletal_minion.strike(mob, facingPC)
			return
		end
	end	
	...
	if mob.registry["anguish_timer"] >= 3 then
		if distanceSquare(mob, target, 7) then
			skeletal_minion.anguish(mob, target)
			return
		end
	end	
	
	
	if mob.registry["haunt_timer"] >= 20 then
		if facingPC ~= nil then
			skeletal_minion.haunt(mob, facingPC)
			return
		end
	end	
...
move = function(mob, target)
	...
	skeletal_minion.magicCast(mob, target)
end,
...
on_spawn = function(mob)
	...
	mob:talk(1,"Skeletal Minion! Who dares enter the Crypt?!")
...
anguish = function(mob, target)
	...
	local anim = 17
	local damage = math.random(180, 286)
