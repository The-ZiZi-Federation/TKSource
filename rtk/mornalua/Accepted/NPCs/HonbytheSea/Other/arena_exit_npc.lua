...
click = async(function(player, npc)
	...
	menu = player:menuString(name.."Are a chicken like me, BAA-CAWWWWWWKKKKKKK!", opts)
...
f1click = function(player, npc)
	...
	menu=player:menuString("<b>[MINIGAME POWERS]\nWhat would you like to  do?", opts)
...
local banned = {}
	...
	for i = 1, #pc do
		if pc[i].registry["minigame_ban_timer"] > os.time() then
...
addPlayer = function(player, target)
	...
	if player.m == 15000 then
		if target.registry["ctf_team"]  == 0 then
			...
			target:playSound(29)
			target:dialogSeq({t, n.."Allright, your character is registered for Flag Freeze Tag."}, 1)
