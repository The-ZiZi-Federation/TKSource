...
click = async(function(player,npc)
	...
	if player.class == bishop and player.mark < 1 and (player.baseHealth >= 457500 or player.baseMagic >= 457500) then table.insert(opts, "Rank 1") end
	--if player.class == bishop and player.mark < 2 and (player.baseHealth >= 1197500 or player.baseMagic >= 1197500) then table.insert(opts, "Rank 2") end
	--if player.class == bishop and player.mark < 3 and (player.baseHealth >= 1970000 or player.baseMagic >= 1970000) then table.insert(opts, "Rank 3") end
	--if player.class == bishop and player.mark < 4 and (player.baseHealth >= 3020000 or player.baseMagic >= 3020000) then table.insert(opts, "Rank 4") end
	--if player.class == bishop and player.mark < 5 and (player.baseHealth >= 4257500 or player.baseMagic >= 4257500) then table.insert(opts, "Rank 5") end
	--if player.class == bishop and player.mark < 5 and (player.baseHealth >= 5682500 or player.baseMagic >= 5682500) then table.insert(opts, "Rank 6") end	

	if player.baseClass == 4 and player.class < 5 then table.insert(opts, "I want to be a Bishop") end
	
	if player.class == bishop then
...
-- Join Path	
	if menu == "I want to be a Bishop" then
		...
		elseif player.level == 110 then
			player:dialogSeq({t, name.."Well isn't that a bit blunt and forward of you to just request to join us.",
								name.."You are going to need that kind of confidence to walk the line with us.",
								name.."We are the grandest healers and prophets that any of the gods could have."}, 1)
			confirm = player:menuString("Are you sure you want to be a Bishop. You can not undo this choice!", {"Yes", "No"})
            if confirm == "Yes" then
				player:dialogSeq({t, name.."Without people like us the world will never be saved. We can not guarantee the side we fight for is always right.",
									name.."So it is up to each of us to determine whether the cause we fight for feels right."}, 1)
				broadcast(-1, "[Congratulations] "..player.name.." is now a Cleric!")
				specRebirth(player, bishop)
