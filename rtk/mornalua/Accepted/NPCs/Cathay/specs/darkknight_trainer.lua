...
click = async(function(player,npc)
	...
	if player.class == darkknight and player.mark < 1 and (player.baseHealth >= 457500 or player.baseMagic >= 457500) then table.insert(opts, "Rank 1") end
	--if player.class == darkknight and player.mark < 2 and (player.baseHealth >= 1197500 or player.baseMagic >= 1197500) then table.insert(opts, "Rank 2") end
	--if player.class == darkknight and player.mark < 3 and (player.baseHealth >= 1970000 or player.baseMagic >= 1970000) then table.insert(opts, "Rank 3") end
	--if player.class == darkknight and player.mark < 4 and (player.baseHealth >= 3020000 or player.baseMagic >= 3020000) then table.insert(opts, "Rank 4") end
	--if player.class == darkknight and player.mark < 5 and (player.baseHealth >= 4257500 or player.baseMagic >= 4257500) then table.insert(opts, "Rank 5") end
	--if player.class == darkknight and player.mark < 5 and (player.baseHealth >= 5682500 or player.baseMagic >= 5682500) then table.insert(opts, "Rank 6") end
	
	if player.baseClass == 1 and player.class < 5 then table.insert(opts, "I want to be a DarkKnight") end
	
	if player.class == darkknight then
...
-- Join Path
	if menu == "I want to be a DarkKnight" then
		...
		elseif player.level == 110 then
			player:dialogSeq({t, name.."Everyone wants to be a DarkKnight! We crush our foes with mighty weapons and dark powers.",
								name.."Can you feel the hatred? Can you smell the blood? Let it awaken the darkness that sleeps inside you.",
								name.."You knew it before you arrived. This is the path to the power you seek."}, 1)
			confirm = player:menuString("Are you sure you want to be a DarkKnight. You can not undo this choice!", {"Yes", "No"})
            if confirm == "Yes" then
				player:dialogSeq({t, name.."This is the first step in your destiny, great one.",
									name.."Today you are a Thrall. In time, you will take Thralls of your own. For now, you serve me."}, 1)
				broadcast(-1, "[Congratulations] "..player.name.." is now a Thrall!")
				specRebirth(player, darkknight)
