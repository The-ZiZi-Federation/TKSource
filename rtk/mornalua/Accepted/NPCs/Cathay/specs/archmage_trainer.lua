...
click = async(function(player,npc)
	...
	if player.class == archmage and player.mark < 1 and (player.baseHealth >= 457500 or player.baseMagic >= 457500) then table.insert(opts, "Rank 1") end
	--if player.class == archmage and player.mark < 2 and (player.baseHealth >= 1197500 or player.baseMagic >= 1197500) then table.insert(opts, "Rank 2") end
	--if player.class == archmage and player.mark < 3 and (player.baseHealth >= 1970000 or player.baseMagic >= 1970000) then table.insert(opts, "Rank 3") end
	--if player.class == archmage and player.mark < 4 and (player.baseHealth >= 3020000 or player.baseMagic >= 3020000) then table.insert(opts, "Rank 4") end
	--if player.class == archmage and player.mark < 5 and (player.baseHealth >= 4257500 or player.baseMagic >= 4257500) then table.insert(opts, "Rank 5") end
	--if player.class == archmage and player.mark < 5 and (player.baseHealth >= 5682500 or player.baseMagic >= 5682500) then table.insert(opts, "Rank 6") end	

	if player.baseClass == 3 and player.class < 5 then table.insert(opts, "I want to be an Archmage") end
	
	if player.class == archmage then
...
-- Join Path
	if menu == "I want to be an Archmage" then
		...
		elseif player.level == 110 then
			player:dialogSeq({t, name.."Archmages are the greatest casters of Morna. With just our mind we can clear any path.",
								name.."Becoming an Archmage means a lifetime of knowing you are the most powerful and true.",
								name.."Are you sure you are capable of commiting to a life of knowing no other can ever truly defeat you?"}, 1)
			confirm = player:menuString("Are you sure you want to be an Archmage. You can not undo this choice!", {"Yes", "No"})
            if confirm == "Yes" then
				player:dialogSeq({t, name.."Welcome to the coven of greatness. There is just one thing you need to know...",
									name.."You will need to start at the bottom and work your way into the power. No shortcuts!"}, 1)
				broadcast(-1, "[Congratulations] "..player.name.." is now a Magi!")
				specRebirth(player,  archmage)
