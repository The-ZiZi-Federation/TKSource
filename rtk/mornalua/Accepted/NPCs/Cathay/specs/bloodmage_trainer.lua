...
click = async(function(player,npc)
	...
	if player.class == bloodmage and player.mark < 1 and (player.baseHealth >= 457500 or player.baseMagic >= 457500) then table.insert(opts, "Rank 1") end
	--if player.class == bloodmage and player.mark < 2 and (player.baseHealth >= 1197500 or player.baseMagic >= 1197500) then table.insert(opts, "Rank 2") end
	--if player.class == bloodmage and player.mark < 3 and (player.baseHealth >= 1970000 or player.baseMagic >= 1970000) then table.insert(opts, "Rank 3") end
	--if player.class == bloodmage and player.mark < 4 and (player.baseHealth >= 3020000 or player.baseMagic >= 3020000) then table.insert(opts, "Rank 4") end
	--if player.class == bloodmage and player.mark < 5 and (player.baseHealth >= 4257500 or player.baseMagic >= 4257500) then table.insert(opts, "Rank 5") end
	--if player.class == bloodmage and player.mark < 5 and (player.baseHealth >= 5682500 or player.baseMagic >= 5682500) then table.insert(opts, "Rank 6") end	

	if player.baseClass == 3 and player.class < 5 then table.insert(opts, "I want to be a Bloodmage") end
	
	if player.class == bloodmage then
...
-- Join Path
	if menu == "I want to be a Bloodmage" then
		...
		elseif player.level == 110 then
			player:dialogSeq({t, name.."You do not just get to be a Bloodmage. You need to earn it.",
								name.."I got a couple ideas of how you can impress me.",
								name.."You will start by swallowing this capsule and agreeing to join us for life."}, 1)
			confirm = player:menuString("Are you sure you want to be a Bloodmage. You can not undo this choice!", {"Yes", "No"})
            if confirm == "Yes" then
				player:dialogSeq({t, name.."There is no turning back now. This is going to be the rest of your life.",
									name.."We will make a Bloodmage out of you yet! In the name of Crucimigrator I Assimilate this follower!"}, 1)
				broadcast(-1, "[Congratulations] "..player.name.." has become an Occultist!")
				specRebirth(player, bloodmage)
			end
		end    
