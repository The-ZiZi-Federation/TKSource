...
click = async(function(player,npc)
	...
	if player.class == bard and player.mark < 1 and (player.baseHealth >= 457500 or player.baseMagic >= 457500) then table.insert(opts, "Rank 1") end
	--if player.class == bard and player.mark < 2 and (player.baseHealth >= 1197500 or player.baseMagic >= 1197500) then table.insert(opts, "Rank 2") end
	--if player.class == bard and player.mark < 3 and (player.baseHealth >= 1970000 or player.baseMagic >= 1970000) then table.insert(opts, "Rank 3") end
	--if player.class == bard and player.mark < 4 and (player.baseHealth >= 3020000 or player.baseMagic >= 3020000) then table.insert(opts, "Rank 4") end
	--if player.class == bard and player.mark < 5 and (player.baseHealth >= 4257500 or player.baseMagic >= 4257500) then table.insert(opts, "Rank 5") end
	--if player.class == bard and player.mark < 5 and (player.baseHealth >= 5682500 or player.baseMagic >= 5682500) then table.insert(opts, "Rank 6") end	

	if player.baseClass == 2 and player.class < 5 then table.insert(opts, "I want to be a Bard") end
...
-- Join Path
	if menu == "I want to be a Bard" then
		...
		elseif player.level == 110 then
			player:dialogSeq({t, name.."Let me guess, you're not Dexterous enough to be an Assassin...",
								name.."No, you're not strong enough to be a Mercenary...",
								name.."No, I can see it. You are smart enough to know this is the best life for people like us."}, 1)
			confirm = player:menuString("Are you sure you want to be a Bard. You can not undo this choice!", {"Yes", "No"})
            if confirm == "Yes" then
				player:dialogSeq({t, name.."Welcome to the show. Hope you like music.",
									name.."You'll be singing sweet melodies in no time!"}, 1)
				broadcast(-1, "[Congratulations] "..player.name.." is now a Troubadour!")
				specRebirth(player,  bard)
