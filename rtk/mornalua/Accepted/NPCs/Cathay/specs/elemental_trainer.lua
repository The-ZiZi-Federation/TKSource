...
click = async(function(player,npc)
	...
	if player.class == elemental and player.mark < 1 and (player.baseHealth >= 457500 or player.baseMagic >= 457500) then table.insert(opts, "Rank 1") end
	--if player.class == elemental and player.mark < 2 and (player.baseHealth >= 1197500 or player.baseMagic >= 1197500) then table.insert(opts, "Rank 2") end
	--if player.class == elemental and player.mark < 3 and (player.baseHealth >= 1970000 or player.baseMagic >= 1970000) then table.insert(opts, "Rank 3") end
	--if player.class == elemental and player.mark < 4 and (player.baseHealth >= 3020000 or player.baseMagic >= 3020000) then table.insert(opts, "Rank 4") end
	--if player.class == elemental and player.mark < 5 and (player.baseHealth >= 4257500 or player.baseMagic >= 4257500) then table.insert(opts, "Rank 5") end
	--if player.class == elemental and player.mark < 5 and (player.baseHealth >= 5682500 or player.baseMagic >= 5682500) then table.insert(opts, "Rank 6") end	

	if player.baseClass == 3 and player.class < 5 then table.insert(opts, "I want to be an Elemental") end
	
	if player.class == elemental then
...
-- Join Path
	if menu == "I want to be an Elemental" then
		...
		elseif player.level == 110 then
			player:dialogSeq({t, name.."Morna provides us with everything we need to live.",
								name.."Knowing this, it is safe to say Morna has more power than any mortal.",
								name.."Dedicating to Elemental Magics is a life of "}, 1)
			confirm = player:menuString("Are you sure you want to be an Elemental. You can not undo this choice!", {"Yes", "No"})
            if confirm == "Yes" then
				player:dialogSeq({t, name.."Morna will thank you herself one day I am sure.",
									name.."Protect this world and she will protect you!"}, 1)
				broadcast(-1, "[Congratulations] "..player.name.." has become a Naturist!")
				specRebirth(player, elemental)
			end
		end    
