...
click = async(function(player,npc)
	...
	if player.class == assassin and player.mark < 1 and (player.baseHealth >= 457500 or player.baseMagic >= 457500) then table.insert(opts, "Rank 1") end
	--if player.class == assassin and player.mark < 2 and (player.baseHealth >= 1197500 or player.baseMagic >= 1197500) then table.insert(opts, "Rank 2") end
	--if player.class == assassin and player.mark < 3 and (player.baseHealth >= 1970000 or player.baseMagic >= 1970000) then table.insert(opts, "Rank 3") end
	--if player.class == assassin and player.mark < 4 and (player.baseHealth >= 3020000 or player.baseMagic >= 3020000) then table.insert(opts, "Rank 4") end
	--if player.class == assassin and player.mark < 5 and (player.baseHealth >= 4257500 or player.baseMagic >= 4257500) then table.insert(opts, "Rank 5") end
	--if player.class == assassin and player.mark < 5 and (player.baseHealth >= 5682500 or player.baseMagic >= 5682500) then table.insert(opts, "Rank 6") end	

	if player.baseClass == 2 and player.class < 5 then table.insert(opts, "I want to be an Assassin") end
...
-- Join Path
	if menu == "I want to be an Assassin" then
		...
		elseif player.level == 110 then
			player:dialogSeq({t, name.."You're here to join the Assassin's Guild? We are an elite society of professional killers.",
								name.."Assassins know there is no room for honor in battle, and specialize in seeking out weakness to exploit. If we can't find a weakness, we create one.",
								name.."Are you ready for a lifetime of murder, guilt-free?"}, 1)
			confirm = player:menuString("Are you sure you want to be an Assassin. You can not undo this choice!", {"Yes", "No"})
            if confirm == "Yes" then
				player:dialogSeq({t, name.."It doesn't matter how good you think you are now, you'll be starting at the bottom like everyone else.",
									name.."You may not realize it yet, but this is a big opportunity. Don't screw it up."}, 1)
				broadcast(-1, "[Congratulations] "..player.name.." is now a Cutthroat!")
				specRebirth(player, assassin)
			end
		end    
