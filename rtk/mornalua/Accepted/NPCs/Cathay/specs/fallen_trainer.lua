...
click = async(function(player,npc)
	...
	if player.class == fallen and player.mark < 1 and (player.baseHealth >= 457500 or player.baseMagic >= 457500) then table.insert(opts, "Rank 1") end
	--if player.class == fallen and player.mark < 2 and (player.baseHealth >= 1197500 or player.baseMagic >= 1197500) then table.insert(opts, "Rank 2") end
	--if player.class == fallen and player.mark < 3 and (player.baseHealth >= 1970000 or player.baseMagic >= 1970000) then table.insert(opts, "Rank 3") end
	--if player.class == fallen and player.mark < 4 and (player.baseHealth >= 3020000 or player.baseMagic >= 3020000) then table.insert(opts, "Rank 4") end
	--if player.class == fallen and player.mark < 5 and (player.baseHealth >= 4257500 or player.baseMagic >= 4257500) then table.insert(opts, "Rank 5") end
	--if player.class == fallen and player.mark < 5 and (player.baseHealth >= 5682500 or player.baseMagic >= 5682500) then table.insert(opts, "Rank 6") end	
	
	if player.baseClass == 4 and player.class < 5 then table.insert(opts, "I want to be a Fallen") end
	
	if player.class == fallen then
...
-- Join Path	
	if menu == "I want to be a Fallen" then
		...
		elseif player.level == 110 then
			player:dialogSeq({t, name.."Not finding what you sought in the priesthood? I can tell.",
								name.."You don't seem like white armor and vows of purity are your calling.",
								name.."You are meant for something different. Something greater. We know the way, you only need to follow."}, 1)
			confirm = player:menuString("Are you sure you want to be a Fallen. You can not undo this choice!", {"Yes", "No"})
            if confirm == "Yes" then
				player:dialogSeq({t, name.."This was pre-ordained by very great forces. You coming here was no accident.",
									name.."Go! Spread the word, begin your new life, Heretic."}, 1)
				broadcast(-1, "[Congratulations] "..player.name.." has become a Heretic!")
				specRebirth(player, fallen)
			end
		end
