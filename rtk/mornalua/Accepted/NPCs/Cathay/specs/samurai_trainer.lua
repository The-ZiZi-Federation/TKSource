...
click = async(function(player,npc)
	...
	if player.class == samurai and player.mark < 1 and (player.baseHealth >= 457500 or player.baseMagic >= 457500) then table.insert(opts, "Rank 1") end
	--if player.class == samurai and player.mark < 2 and (player.baseHealth >= 1197500 or player.baseMagic >= 1197500) then table.insert(opts, "Rank 2") end
	--if player.class == samurai and player.mark < 3 and (player.baseHealth >= 1970000 or player.baseMagic >= 1970000) then table.insert(opts, "Rank 3") end
	--if player.class == samurai and player.mark < 4 and (player.baseHealth >= 3020000 or player.baseMagic >= 3020000) then table.insert(opts, "Rank 4") end
	--if player.class == samurai and player.mark < 5 and (player.baseHealth >= 4257500 or player.baseMagic >= 4257500) then table.insert(opts, "Rank 5") end
	--if player.class == samurai and player.mark < 5 and (player.baseHealth >= 5682500 or player.baseMagic >= 5682500) then table.insert(opts, "Rank 6") end
	
	if player.baseClass == 1 and player.class < 5 then table.insert(opts, "I want to be a Samurai") end
	
	if player.class == samurai then
...
-- Join Path
	if menu == "I want to be a Samurai" then
		...
		elseif player.level == 110 then
			player:dialogSeq({t, name.."Becoming a Samurai is choosing the life of the sword.",
								name.."We hold the equalizer in any situation. Strength, Stamina, Speed, Swing power. ",
								name.."Joining us is not to be a Samurai. You join only if you want to be the Greatest Samurai!"}, 1)
			confirm = player:menuString("Are you sure you want to be a Samurai. You can not undo this choice!", {"Yes", "No"})
            if confirm == "Yes" then
				player:dialogSeq({t, name.."Alright then. Welcome Ronin. I can't say that I envy your next steps.",
									name.."Live by the sword. Die by the sword. Never back down!"}, 1)
				broadcast(-1, "[Congratulations] "..player.name.." has become a Ronin!")
				specRebirth(player, samurai)
			end
		end    
