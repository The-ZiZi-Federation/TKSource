...
calcHighestThreat=function(mob)
	...
	local mobTargetFacing = getTargetFacing(mob, BL_MOB)
	
	local eMob = mob:getObjectsInCell(mob.m, mob.x+1, mob.y, BL_MOB)
	local fMob = mob:getObjectsInCell(mob.m, mob.x, mob.y-1, BL_MOB)
	local gMob = mob:getObjectsInCell(mob.m, mob.x-1, mob.y, BL_MOB)
	local hMob = mob:getObjectsInCell(mob.m, mob.x, mob.y+1, BL_MOB)
...
--[[	
	...
	if (current_target > 0) then
		if mob.registry["priority_target"] > 0 then
			p = Player(mob.registry["priority_target"])
			if p ~= nil then
				if p.m == mob.m then
					mob.target = p.ID
				else
	...
	if (mob.target > 0 and mobTargetFacing ~= nil and mobTargetFacing.ID ~= mob.target and not mob:checkToObject(mob:getBlock(mob.target)) and (#eMob > 0 or #fMob > 0 or #gMob > 0 or #hMob > 0)) then -- or #i > 0 or #j > 0 or #k > 0 or #l > 0)) then
		if (#eMob > 0) then
			for a = 1, #eMob do
				if (eMob[a].m == mob.m and mob:checkThreat(eMob[a].ID) > highest_threat) then
					highest_threat = mob:checkThreat(eMob[a].ID)
				--	current_target = eMob[a].ID
				end
			end
		end
		if (#fMob > 0) then
			for a = 1, #fMob do
				if (fMob[a].m == mob.m and mob:checkThreat(fMob[a].ID) > highest_threat) then
					highest_threat = mob:checkThreat(fMob[a].ID)
				--	current_target = fMob[a].ID
				end
			end
		end
		if (#gMob > 0) then
			for a = 1, #gMob do
				if (gMob[a].m == mob.m and mob:checkThreat(gMob[a].ID) > highest_threat) then
					highest_threat = mob:checkThreat(gMob[a].ID)
				--	current_target = gMob[a].ID
				end
			end
		end
		if (#hMob > 0) then
			for a = 1, #hMob do
				if (hMob[a].m == mob.m and mob:checkThreat(hMob[a].ID) > highest_threat) then
					highest_threat = mob:checkThreat(hMob[a].ID)
				--	current_target = hMob[a].ID
				end
			end
		end
	else
	...
	if mob.target == mob.ID then mob.target = 0 end
end,
