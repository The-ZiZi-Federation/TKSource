...
--mob:talk(0,"target: "..mob.target)
	if (not canMove(mob)) then
		...
		if (distance(mob, target) <= 4) then	
			RunAway(mob, target)
		elseif (distance(mob, target) > 5) then		
	...
	if (distance(mob, target) <= 6 and mob.registry["gollum_throw_stone_aether"] < os.time() and mob.m == target.m) then
		gollum.throw_stone(mob, target)
	end
	
	if (distance(mob, target) <= 10 and mob.registry["gollum_stone_connection_aether"] < os.time() and target.state ~= 1 and mob.m == target.m) then
...
mob:talk(0,"wave start")
	...
	around_push_stun.aroundCheck(mob, BL_PC, 1)
	around_push_stun.aroundCheck(mob, BL_MOB, 1)
	
	--fourPush(mob, BL_PC, 1)
	--fourPush(mob, BL_MOB, 1)
	
	mob:talk(0,"wave fail")
...
earth_stomp = function(mob, target)
	...
	if (#pcBlocks > 0) then
		for i = 1, #pcBlocks do
			if (distance(mob, pcBlocks[i]) <= 10 and pcBlocks[i].state ~= 1 and pcBlocks[i].m == mob.m) then
...
gollum_buff = {
cast = function(mob)
mob:talk(0,"buff cast")
	mob:setDuration("gollum_buff", 6000)
	mob.armor = mob.armor + 1000
end,

recast = function(mob)
mob:talk(0,"buff recast")
	mob.armor = mob.armor + 1000
end,
