...
on_learn = function(player) player.registry["light"] = 1 end,
on_forget = function(player) player.registry["light"] = 0 end,
...
cast = function(player, target)
	...
	if target:hasDuration("light") then alreadyCast(player) return end
	...
	if target.blType == BL_PC then
		...
		if target.ID ~= player.ID then target:sendMinitext(player.name.." cast Light on you") end
	end
end,
