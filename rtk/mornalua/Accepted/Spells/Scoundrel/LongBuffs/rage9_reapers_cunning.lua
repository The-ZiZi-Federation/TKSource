...
--                  . . . 
-------------------------------------------------------
--    Desc: 42x your damage output to swing and spells while cast.
...
on_learn = function(player) player.registry["learned_reapers_cunning"] = 1

end,
on_forget = function(player) player.registry["learned_reapers_cunning"] = 0 end,

cast = function(player)

	local magicCost = 45000
	local duration = 600000
	local sound = 31
	local anim = 35

	if not player:canCast(1,1,0) then return end
	if player:hasDuration("reapers_cunning") then player:setDuration("reapers_cunning", 0) return end	
	...
	player:setDuration("reapers_cunning", duration)
	player:calcStat()
	player:sendMinitext("You cast Reaper's Cunning")
end,
...
requirements = function(player)
	...
	local desc = {"Reaper's Cunning is a stronger version of Silhouette's Cunning.", txt}
