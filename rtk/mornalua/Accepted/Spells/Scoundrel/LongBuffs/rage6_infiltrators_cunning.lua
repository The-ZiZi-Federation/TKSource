...
--                  . . . 
-------------------------------------------------------
--    Desc: 23x your damage output to swing and spells while cast.
--        : Gain flanking and the ability to hit up to four targets at once.
-------------------------------------------------------
-- Script Author: John Day / John Crandell / Justin Chartier
--   Last Edited: 6/5/2017
-------------------------------------------------------
infiltrators_cunning = {

on_learn = function(player) player.registry["learned_infiltrators_cunning"] = 1

end,
on_forget = function(player) player.registry["learned_infiltrators_cunning"] = 0 end,

cast = function(player)

	local magicCost = 4200
	local duration = 600000
	local sound = 31
	local anim = 35

	if not player:canCast(1,1,0) then return end
	if player:hasDuration("infiltrators_cunning") then player:setDuration("infiltrators_cunning", 0) return end	
	...
	player:setDuration("infiltrators_cunning", duration)
	player:calcStat()
	player:sendMinitext("You cast Infiltrator's Cunning")
end,
...
requirements = function(player)
	...
	local desc = {"Infiltrator's Cunning is a stronger version of Rogue's Cunning.", txt}
