...
--                  . . . 
-------------------------------------------------------
--    Desc: 160x your damage output to swing and spells while cast.
...
on_learn = function(player) player.registry["learned_deaths_cunning"] = 1

end,
on_forget = function(player) player.registry["learned_deaths_cunning"] = 0 end,

cast = function(player)

	local magicCost = 225000
	local duration = 600000
	local sound = 31
	local anim = 35

	if not player:canCast(1,1,0) then return end
	if player:hasDuration("deaths_cunning") then player:setDuration("deaths_cunning", 0) return end	
	...
	player:setDuration("deaths_cunning", duration)
	player:calcStat()
	player:sendMinitext("You cast Death's Cunning")
end,
...
requirements = function(player)
	...
	local desc = {"Death's Cunning is a stronger version of Demon's Cunning.", txt}
