...
--                  . . . 
-------------------------------------------------------
--    Desc: 28x your damage output to swing and spells while cast.
...
on_learn = function(player) player.registry["learned_shades_cunning"] = 1

end,
on_forget = function(player) player.registry["learned_shades_cunning"] = 0 end,
...
cast = function(player)
	...
	if player:hasDuration("shades_cunning") then player:setDuration("shades_cunning", 0) return end	
	...
	player:setDuration("shades_cunning", duration)
	player:calcStat()
	player:sendMinitext("You cast Shade's Cunning")
end,
...
requirements = function(player)
	...
	local desc = {"Shade's Cunning is a stronger version of Infiltrator's Cunning.", txt}
