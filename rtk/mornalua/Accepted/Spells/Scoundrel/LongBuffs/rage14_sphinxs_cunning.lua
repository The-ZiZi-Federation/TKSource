...
--                  . . . 
-------------------------------------------------------
--    Desc: 100x your damage output to swing and spells while cast.
...
on_learn = function(player) player.registry["learned_sphinxs_cunning"] = 1
end,
on_forget = function(player) player.registry["learned_sphinxs_cunning"] = 0 end,

cast = function(player)

	local magicCost = 150000
	local duration = 600000
	local sound = 31
	local anim = 35

	if not player:canCast(1,1,0) then return end
	if player:hasDuration("sphinxs_cunning") then player:setDuration("sphinxs_cunning", 0) return end	
	...
	player:setDuration("sphinxs_cunning", duration)
	player:calcStat()
	player:sendMinitext("You cast Sphinx's Cunning")
end,
...
requirements = function(player)
	...
	local desc = {"Sphinx's Cunning is a stronger version of Rakshasa's Cunning.", txt}
