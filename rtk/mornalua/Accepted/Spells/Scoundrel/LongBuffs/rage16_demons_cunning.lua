...
--                  . . . 
-------------------------------------------------------
--    Desc: 138x your damage output to swing and spells while cast.
...
on_learn = function(player) player.registry["learned_demons_cunning"] = 1

end,
on_forget = function(player) player.registry["learned_demons_cunning"] = 0 end,

cast = function(player)

	local magicCost = 200000
	local duration = 600000
	local sound = 31
	local anim = 35

	if not player:canCast(1,1,0) then return end
	if player:hasDuration("demons_cunning") then player:setDuration("demons_cunning", 0) return end	
	...
	player:setDuration("demons_cunning", duration)
	player:calcStat()
	player:sendMinitext("You cast Demon's Cunning")
end,
...
requirements = function(player)
	...
	local desc = {"Demon's Cunning is a stronger version of Djinn's Cunning.", txt}
