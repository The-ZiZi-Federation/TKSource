...
--                  . . . 
-------------------------------------------------------
--    Desc: 83x your damage output to swing and spells while cast.
...
on_learn = function(player) player.registry["learned_rakshasas_cunning"] = 1

end,
on_forget = function(player) player.registry["learned_rakshasas_cunning"] = 0 end,

cast = function(player)

	local magicCost = 125000
	local duration = 600000
	local sound = 31
	local anim = 35

	if not player:canCast(1,1,0) then return end
	if player:hasDuration("rakshasas_cunning") then player:setDuration("rakshasas_cunning", 0) return end	
	...
	player:setDuration("rakshasas_cunning", duration)
	player:calcStat()
	player:sendMinitext("You cast Rakshasa's Cunning")
end,
...
requirements = function(player)
	...
	local desc = {"Rakshasa's Cunning is a stronger version of Hellcat's Cunning.", txt}
