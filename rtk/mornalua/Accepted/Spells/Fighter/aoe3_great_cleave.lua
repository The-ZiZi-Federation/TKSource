-------------------------------------------------------
--   Spell: Great Cleave (Whirlwind Attack Lvl. 3)                        
--   Class: Fighter
--   Level: 95
--  Aether: 18 Seconds
...
on_learn = function(player) 
    ...
    on_forget = function(player) player.registry["learned_great_cleave"] = 0 end,
...
----------------------
	...
	if player.blType == BL_PC then
		...
		player:setAether("great_cleave", aether)
		...
		for i = 1, 12 do
			if (mobflankTargets[i] ~= nil) then
				...
				if player.baseClass == 2 and mobflankTargets[i]:hasDuration("stun") then
					mobflankTargets[i]:removeHealthExtend(damage2, 1, 1, 0, 1, 1)
				else
