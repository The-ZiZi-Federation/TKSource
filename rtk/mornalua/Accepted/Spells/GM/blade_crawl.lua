...
cast = function(player)
	...
	if 	not player:canCast(1,1,0) then return else
		if player.health < minAmount then notEnoughHP(player) return else
			if player.magic < minAmount then notEnoughMP(player) return else
				if player:hasDuration("blade_crawl") then anim(player) return else
...
action = function(block, owner)
	...
	if step == 0 then block.registry["step"] = 1 else
		if step < 7 then block.registry["step"] = step + 1 else endAction(block) end
	end
	...
	if side == 0 then y = y-step end
	if side == 1 then x = x+step end
	if side == 2 then y = y+step end
	if side == 3 then x = x-step end
	block:sendAnimationXY(411, x, y)
	owner:playSound(357)
end,

endAction = function(block) block:delete() end,
