...
canCast = function(player, block)
	...
	return else
		if block:hasDuration("gm_push") then return else
			for i = 0, 3 do block.registry["gm_push"..i] = 0 end
...
--	player:sendAnimationXY(15, player.x, player.y)
	...
	if player.registry["gm_push0"] == 1 then
		if getPass(player.m, player.x, player.y-1) == 1 then player:setDuration("gm_push", 0) player:sendAnimation(153) player:playSound(59) return else
			player:warp(player.m, player.x, player.y-1)
		end
	elseif player.registry["gm_push1"] == 1 then
		if getPass(player.m, player.x+1, player.y) == 1 then player:setDuration("gm_push", 0) player:sendAnimation(153) player:playSound(59) return else
