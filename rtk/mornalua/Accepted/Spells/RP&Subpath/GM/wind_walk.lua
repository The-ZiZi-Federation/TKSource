...
cast = function(player)
	...
	if #mob > 0 then
		for i = 1, #mob do
			if mob[i].yname == "wind_walk" then table.insert(same, mob[i].ID) end
		end
	end
	if #same > 20 then anim(player) player:sendMinitext("Can't over than 20 clones in 1 map") return false end
		...
		for i = 1, #get do
			if get[i].yname == "wind_walk" then
				if player.gfxClone == 0 then
					...
					if player.registry["show_title"] == 1 then get[i].gfxName = player.title.." "..player.name else
