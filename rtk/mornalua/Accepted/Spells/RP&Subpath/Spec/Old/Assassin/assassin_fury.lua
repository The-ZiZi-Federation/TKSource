...
cast = function(player)
	...
	if player:hasDuration("combat_clairvoyance") or player:hasDuration("combat_foresight") or player:hasDuration("combat_perception") or player:hasDuration("combat_reflexes") or player:hasDuration("assassin_fury") then
		anim(player)
		player:sendMinitext("Spell is already cast!")
	return else
		if player.fury < 4 then
			player:setDuration("assassin_fury", dura)
			player:sendAction(6, 20)
			player.magic = player.magic - magicCost
			player.fury = 50
			player:sendStatus()
			player:sendAnimation(126)
			player:playSound(9)
			player:sendMinitext("You cast Assassin Fury")
		end
	end
end,
