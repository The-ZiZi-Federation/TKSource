...
on_learn = function(player) player.registry["learned_unholy_blessing"] = 1 end,
on_forget = function(player) player.registry["learned_unholy_blessing"] = 0 end,
...
cast = function(player)
	...
	for i = 1, #player.group do
		if (Player(player.group[i]).state ~= 1 and Player(player.group[i]).m == player.m) then
			...
			Player(player.group[i]):sendAnimation(149)
			...
			Player(player.group[i]):setDuration("unholy_blessing", duration, 0, 1)
			if player.class == 49 then player.fury = 27 end
			Player(player.group[i]):sendMinitext(player.name.." grants you a Dark blessing")
...
uncast = function(player)
	...
	if player.class == 49 then player.fury = 1 end
	player:calcStat()
	player:updateStatus()
	player:sendStatus()
end
