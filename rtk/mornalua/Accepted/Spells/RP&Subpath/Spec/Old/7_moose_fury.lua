...
on_learn = function(player) player.registry["learned_moose_fury"] = 1 end,
on_forget = function(player) player.registry["learned_moose_fury"] = 0 end,

cast = function(player)

	local magicCost = 7500
	local duration = 600000
	local mooseBuffBonus = 5
	...
	player.registry["moose_buff_bonus"] = mooseBuffBonus
	player:sendAction(6, 20)
	player.magic = player.magic - magicCost
	player:sendStatus()
	player:sendAnimation(70)
	player:playSound(9)
	player:setDuration("moose_fury", duration)
	player:calcStat()
	player:sendMinitext("You cast Moose Fury")
...
--Player(4):talk(0,""..player.registry["trance_buff_bonus"])
	...
	local mooseBuffBonus = player.registry["moose_buff_bonus"]
--Player(4):talk(0,""..tranceBuffBonus)

	player.fury = player.fury + mooseBuffBonus
...
uncast = function(player) 
	...
	player.fury = player.fury - mooseBuffBonus
	player:calcStat() 
	player.registry["moose_buff_bonus"] = 0
	player:sendMinitext("Your Moose Fury fades away")
