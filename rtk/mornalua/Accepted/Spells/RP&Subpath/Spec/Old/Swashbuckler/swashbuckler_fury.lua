...
cast = function(player)
	...
	if player:hasDuration("combat_clairvoyance") or player:hasDuration("combat_foresight") or player:hasDuration("combat_perception") or player:hasDuration("combat_reflexes") or player:hasDuration("swashbuckler_fury") then
		anim(player)
		player:sendMinitext("Spell is already cast!")
	return else
		if player.fury < 4 then
			player:setDuration("swashbuckler_fury", dura)
			...
			player:playSound(9)
			player:sendMinitext("You cast Swashbuckler Fury")
		end
	end
end,
