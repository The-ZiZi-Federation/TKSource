...
--            X
-------------------------------------------------------
--    Desc: Rocks fall all around target.
-------------------------------------------------------
...
rock_fall = {
    ...
    on_learn = function(player) player.registry["learned_rock_fall"] = 1 end,
    on_forget = function(player) player.registry["learned_rock_fall"] = 0 end,
...
---------------player Targetting player------------------------------
		elseif (target.blType == BL_PC) then
			...
			player:sendMinitext("You cast Rock Fall")
			player:playSound(sound)
			player:setAether("rock_fall", aether)
			--- What happens to the player target on cast --------------------------------------		
			target:sendMinitext(player.name.." surrounds you with a Rock Fall!")
			target:sendAnimation(anim)
			target:playSound(sound)
			target.registry["rock_fall"] = player.ID
			target:setDuration("rock_fall", duration, player.ID)
		-----------------------------------------------------------------------------
		end
...
--Varable Declarations
----------------------
	local user = Player(player.registry["rock_fall"])
...
--  Get targets around the player Rock Fall was cast upon ------------------------------------
			...
			if distance(player, mob_around[i]) <= 2 then
				if (player.blType == BL_PC) then
					if player.gmLevel > 0 then
						player:sendMinitext("Rock Fall DMG: "..damage)
					end
				end
				rock_fall.takeDamage(player, mob_around[i], damage)
			end
		end
	end
			...
			if distance(player, pc_around[i]) <= 2 then
				if pc_around[i].ID ~= player.ID then
					if pc_around[i].state ~= 1 and player:canPK(pc_around[i]) then
						rock_fall.takeDamage(player, pc_around[i], damage)
					end
				end
			end
		end
...
takeDamage = function(player, target, damage)
	local caster = Player(player.registry["rock_fall"])
	local threat
	local anim = 504
	local sound
	local duration = 4000
...
requirements = function(player)
	...
	local desc = {"Rock Fall is a damage over time Area affect spell!!", txt}
