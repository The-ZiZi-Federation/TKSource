...
on_learn = function(player) player.registry["learned_greater_haunt"] = 1 end,
on_forget = function(player) player.registry["learned_greater_haunt"] = 0 end,

cast = function(player, target)
	

	local magicCost = 0
	local aether = 43200000 --12 hours
	local duration = 900000 --15 minutes
	local sound = 106
...
--		549, --swirling white snakes
	...
	if target ~= nil then
		...
		player:calcStat()
		player:sendStatus()
		player:sendMinitext("You cast Greater Haunt.")
		player:playSound(sound)
		target.registry["haunt_anim"] = randomAnim
		target:sendAnimation(randomAnim)
		target:setDuration("greater_haunt", duration)
		player:setAether("greater_haunt", aether)
...
requirements = function(player)
	...
	local desc = {"Greater Haunt is a spell that orders a wandering spirit to haunt your target.", txt}
