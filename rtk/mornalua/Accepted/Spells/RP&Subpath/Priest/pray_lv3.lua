...
on_learn = function(player) player.registry["learned_pray_lv3"] = 1 
player:removeSpell("pray_lv1") 
player:removeSpell("pray_lv2")
player:removeSpell("pray_lv4") 
end,
on_forget = function(player) player.registry["learned_pray_lv3"] = 0 end,
...
cast = function(player)
	...
	if player.state ~= 1 then
		player:setAether("pray_lv3", aether)
		player:playSound(726)
		player:sendMinitext("You start to pray.")
		player:setDuration("pray_lv3", duration)
...
uncast = function(player)
	...
	player.magic = player.magic + (player.maxMagic*.90)
	player:sendStatus()
	player:updateState()
	player:calcStat()
...
requirements = function(player)
	...
	local level = 95
	local item = {0, 3022, 249}
	local amounts = {50000, 10, 10}
