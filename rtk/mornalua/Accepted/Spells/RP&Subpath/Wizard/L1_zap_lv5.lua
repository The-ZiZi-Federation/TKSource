...
on_learn = function(player) player.registry["learned_zap_lv5"] = 1 
    ...
    on_forget = function(player) player.registry["learned_zap_lv5"] = 0 end,
...
---------------------------------
	...
	player:sendMinitext("You cast Zap Lv5")
	if player.registry["extra_spell_info"] == 1 then
		player:sendMinitext("Zap Lv5 DMG: "..damage)
	end
...
----------------player vs player-----------------------------------------------------------	
	elseif (target.blType == BL_PC and player:canPK(target)) then
		...
		target.attacker = player.ID
		target:removeHealthExtend(damage, 1, 1, 0, 1, 1)
		target:sendMinitext(player.name.." burns you with Zap Lv5")
		if r <= 50 then 
			if not target:hasDuration("shock") then
				target:setDuration("shock", shockDuration)
			end
		...
		for i = 1, 4 do
			if (pcSideTargets[i] ~= nil) then
				if pcSideTargets[i].state ~= 1 and player:canPK(pcSideTargets[i]) then
					...
					pcflankTargets[i]:sendMinitext(player.name.." burns you with Zap Lv5.")
...
requirements = function(player)
	...
	local desc = {"Zap Lv5 electrocutes an opponent!\n\nReplaces Zap lv4", txt}
