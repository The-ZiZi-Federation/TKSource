...
on_learn = function(player) player.registry["learned_snowstorm_lv3"] = 1
    ...
    on_forget = function(player) player.registry["learned_snowstorm_lv3"] = 0 end,
...
-----------------player vs mob-------------------------------
	if (target.blType == BL_MOB) then
		...
		player:sendMinitext("You cast Snowstorm Lv3")
		
		if player.registry["extra_spell_info"] == 1 then
			player:sendMinitext("Snowstorm Lv3 DMG: "..damage)
		end
...
----------------player vs player-----------------------------------------------------------	
	elseif (target.blType == BL_PC and player:canPK(target)) then
		...
		target.attacker = player.ID
		target:removeHealthExtend(damage, 1, 1, 0, 1, 1)
		target:sendMinitext(player.name.." burns you with Snowstorm Lv3")
		if r <= 50 then 
			if not target:hasDuration("slow") then
				target:setDuration("slow", slowDuration)
			end
...
requirements = function(player)
	...
	local desc = {"Snowstorm Lv3 pelts an opponent with snow!\n\nReplaces Snowstorm Lv2", txt}
