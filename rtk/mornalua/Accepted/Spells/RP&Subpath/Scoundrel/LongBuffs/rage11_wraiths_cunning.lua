...
--                  . . . 
-------------------------------------------------------
--    Desc: 60x your damage output to swing and spells while cast.
...
on_learn = function(player) player.registry["learned_wraiths_cunning"] = 1

end,
on_forget = function(player) player.registry["learned_wraiths_cunning"] = 0 end,

cast = function(player)

	local magicCost = 80000
	local duration = 600000
	local sound = 31
	local anim = 35

	if not player:canCast(1,1,0) then return end
	if player:hasDuration("wraiths_cunning") then player:setDuration("wraiths_cunning", 0) return end	
	...
	player:setDuration("wraiths_cunning", duration)
	player:calcStat()
	player:sendMinitext("You cast Wraith's Cunning")
end,
...
requirements = function(player)
	...
	local desc = {"Wraith's Cunning is a stronger version of Tiefling's Cunning.", txt}
