...
--                  . . . 
-------------------------------------------------------
--    Desc: 9x your damage output to swing and spells while cast.
...
on_learn = function(player) player.registry["learned_rogues_cunning"] = 1

end,
on_forget = function(player) player.registry["learned_rogues_cunning"] = 0 end,

cast = function(player)

	local magicCost = 2200
	local duration = 600000
	local sound = 31
	local anim = 35

	if not player:canCast(1,1,0) then return end
	if player:hasDuration("rogues_cunning") then player:setDuration("rogues_cunning", 0) return end	
	...
	player:setDuration("rogues_cunning", duration)
	player:calcStat()
	player:sendMinitext("You cast Rogue's Cunning")
end,
...
requirements = function(player)
	...
	local desc = {"Rogue's Cunning is a stronger version of Vagabond's Cunning.", txt}
