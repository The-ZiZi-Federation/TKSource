...
--                  . . . 
-------------------------------------------------------
--    Desc: 181x your damage output to swing and spells while cast.
...
on_learn = function(player) player.registry["learned_spirits_cunning"] = 1

end,
on_forget = function(player) player.registry["learned_spirits_cunning"] = 0 end,

cast = function(player)

	local magicCost = 250000
	local duration = 600000
	local sound = 31
	local anim = 35

	if not player:canCast(1,1,0) then return end
	if player:hasDuration("spirits_cunning") then player:setDuration("spirits_cunning", 0) return end	
	...
	player:setDuration("spirits_cunning", duration)
	player:calcStat()
	player:sendMinitext("You cast Spirit's Cunning")
end,
...
requirements = function(player)
	...
	local desc = {"Spirit's Cunning is a stronger version of Death's Cunning.", txt}
