...
--                  . . . 
-------------------------------------------------------
--    Desc: 115x your damage output to swing and spells while cast.
...
on_learn = function(player) player.registry["learned_djinns_cunning"] = 1

end,
on_forget = function(player) player.registry["learned_djinns_cunning"] = 0 end,

cast = function(player)

	local magicCost = 175000
	local duration = 600000
	local sound = 31
	local anim = 35

	if not player:canCast(1,1,0) then return end
	if player:hasDuration("djinns_cunning") then player:setDuration("djinns_cunning", 0) return end	
	...
	player:setDuration("djinns_cunning", duration)
	player:calcStat()
	player:sendMinitext("You cast Djinn's Cunning")
end,
...
requirements = function(player)
	...
	local desc = {"Djinn's Cunning is a stronger version of Sphinx's Cunning.", txt}
