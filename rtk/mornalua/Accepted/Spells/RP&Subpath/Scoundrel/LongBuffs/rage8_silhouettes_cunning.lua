...
--                  . . . 
-------------------------------------------------------
--    Desc: 36x your damage output to swing and spells while cast.
...
on_learn = function(player) player.registry["learned_silhouettes_cunning"] = 1

end,
on_forget = function(player) player.registry["learned_silhouettes_cunning"] = 0 end,

cast = function(player)

	local magicCost = 25000
	local duration = 600000
	local sound = 31
	local anim = 35

	if not player:canCast(1,1,0) then return end
	if player:hasDuration("silhouettes_cunning") then player:setDuration("silhouettes_cunning", 0) return end	
	...
	player:setDuration("silhouettes_cunning", duration)
	player:calcStat()
	player:sendMinitext("You cast Silhouette's Cunning")
end,
...
requirements = function(player)
	...
	local desc = {"Silhouette's Cunning is a stronger version of Shade's Cunning.", txt}
