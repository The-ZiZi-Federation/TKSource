...
--                  . . . 
-------------------------------------------------------
--    Desc: 269x your damage output to swing and spells while cast.
...
on_learn = function(player) player.registry["learned_deitys_cunning"] = 1
end,
on_forget = function(player) player.registry["learned_deitys_cunning"] = 0 end,

cast = function(player)

	local magicCost = 400000
	local duration = 600000
	local sound = 31
	local anim = 35

	if not player:canCast(1,1,0) then return end
	if player:hasDuration("deitys_cunning") then player:setDuration("deitys_cunning", 0) return end	
	...
	player:setDuration("deitys_cunning", duration)
	player:calcStat()
	player:sendMinitext("You cast Deity's Cunning")
end,
...
requirements = function(player)
	...
	local desc = {"Deity's Cunning is a stronger version of Demigod's Cunning.", txt}
