...
--                  . . . 
-------------------------------------------------------
--    Desc: 200x your damage output to swing and spells while cast.
...
on_learn = function(player) player.registry["learned_immortals_cunning"] = 1

end,
on_forget = function(player) player.registry["learned_immortals_cunning"] = 0 end,

cast = function(player)

	local magicCost = 300000
	local duration = 600000
	local sound = 31
	local anim = 35

	if not player:canCast(1,1,0) then return end
	if player:hasDuration("immortals_cunning") then player:setDuration("immortals_cunning", 0) return end	
	...
	player:setDuration("immortals_cunning", duration)
	player:calcStat()
	player:sendMinitext("You cast Immortal's Cunning")
end,
...
requirements = function(player)
	...
	local desc = {"Immortal's Cunning is a stronger version of Spirit's Cunning.", txt}
