...
on_learn = function(player) player.registry["learned_asaks_blessing"] = 1 end,
on_forget = function(player) player.registry["learned_asaks_blessing"] = 0 end,
...
cast = function(player)
	...
	for i = 1, #player.group do
		if (Player(player.group[i]).state ~= 1 and Player(player.group[i]).m == player.m) then
			...
			Player(player.group[i]):sendAnimation(117)
			Player(player.group[i]):sendAnimation(130)
			Player(player.group[i]):sendAnimation(132)
			Player(player.group[i]):sendAnimation(133)
			Player(player.group[i]).basemight = Player(player.group[i]).basemight + 50
			Player(player.group[i]).basewill = Player(player.group[i]).basewill + 80
			Player(player.group[i]).basegrace = Player(player.group[i]).basegrace + 50
			Player(player.group[i]):sendStatus()
			Player(player.group[i]):calcStat()
			Player(player.group[i]):setDuration("asaks_blessing", duration, 0, 1)
			if Player(player.group[i]).class == 44 then Player(player.group[i]).fury = 20 end
...
uncast = function(player)
	player.basemight = player.basemight - 50
	player.basewill = player.basewill - 80
	player.basegrace = player.basegrace - 50
	if player.class == 44 then player.fury = 1 end
	player:calcStat()
	player:updateStatus()
	player:sendStatus()
end
