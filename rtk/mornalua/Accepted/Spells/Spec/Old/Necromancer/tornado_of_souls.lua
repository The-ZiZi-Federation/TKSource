...
tornado_of_souls = {
    ...
    on_learn = function(player) player.registry["learned_tornado_of_souls"] = 1 end,
    on_forget = function(player) player.registry["learned_tornado_of_souls"] = 0 end,
...
cast = function(player, target)
	...
	if (target.blType == BL_MOB) then
		...
		player:sendStatus()
		player:setAether("tornado_of_souls", 1000)
		player:playSound(58)
		target:sendAnimation(291)
		player:sendMinitext("You cast Tornado of Souls")
	...
	elseif (target.blType == BL_PC) then
		...
		if player:canPK(target) then
			target:removeHealthExtend(damage, 1, 1, 1, 1, 0)
			target:sendMinitext(player.name.." haunts you with a Tornado of Souls")
		end
	end
