...
-- level 6 mage F1 snowstorm2 single target zap Lv1
--[[
snowstorm2 = {

    on_learn = function(player) player.registry["learned_snowstorm2"] = 1 end,
    on_forget = function(player) player.registry["learned_snowstorm2"] = 0 end,
...
cast = function(player, target)
	...
	if (target.blType == BL_MOB) then
		...
		player:sendStatus()
		player:setAether("snowstorm2", aether)
		player:playSound(sound)
		target:sendAnimation(anim)
		player:sendMinitext("You cast Snowstorm Lv2.")
		target.attacker = player.ID
		if player:hasDuration("flurry") then target:setDuration("stun", 1000) end
	...
	if mobTargetNorth ~= nil then
		...
		if player:hasDuration("flurry") then mobTargetNorth[1]:setDuration("stun", 1000) end
	...
	if mobTargetSouth ~= nil then
		...
		if player:hasDuration("flurry") then mobTargetSouth[1]:setDuration("stun", 1000) end
	...
	if mobTargetEast ~= nil then
		...
		if player:hasDuration("flurry") then mobTargetEast[1]:setDuration("stun", 1000) end
	...
	if mobTargetWest ~= nil then
		...
		if player:hasDuration("flurry") then mobTargetWest[1]:setDuration("stun", 1000) end
