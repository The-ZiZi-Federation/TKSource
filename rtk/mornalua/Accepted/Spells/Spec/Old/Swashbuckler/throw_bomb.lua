...
throw_bomb = {
    ...
    on_learn = function(player) player.registry["learned_throw_bomb"] = 1 end,
    on_forget = function(player) player.registry["learned_throw_bomb"] = 0 end,
...
cast = function(player, target)
	...
	if (target.blType == BL_MOB) then
		...
		player:sendStatus()
		player:setAether("throw_bomb", 35000)
		player:playSound(58)
		target:sendAnimation(188)
		target:sendAnimation(415)
	--	target:sendAnimation(19)
		player:sendMinitext("You throw a bomb!")
	...
	--	target:sendAnimation(19)
		player:sendMinitext("You cast throw_bomb.")
		target.attacker = player.ID
		if player:canPK(target) then
			target:removeHealthExtend(damage, 1, 1, 1, 1, 0)
			target:sendMinitext(player.name.." throws a bomb at you")
		end
	end
