...
-------------------------------------------------------
strength_of_the_bear = {

on_learn = function(player) player.registry["learned_strength_of_the_bear"] = 1 end,
on_forget = function(player) player.registry["learned_strength_of_the_bear"] = 0 end,

cast = function(player)

	local magicCost = 7500
	local duration = 600000
	local bearBuffBonus = 5
	...
	if player:hasDuration("strength_of_the_bear") then
		player:setDuration("strength_of_the_bear", 0)
		return
	end
	...
	player.registry["bear_buff_bonus"] = bearBuffBonus
	player:sendAction(6, 20)
	player.magic = player.magic - magicCost
	player:sendStatus()
	player:sendAnimation(70)
	player:playSound(9)
	player:setDuration("strength_of_the_bear", duration)
	player:calcStat()
	player:sendMinitext("You cast Strength of the Bear")
...
--Player(4):talk(0,""..player.registry["trance_buff_bonus"])
	...
	local bearBuffBonus = player.registry["bear_buff_bonus"]
--Player(4):talk(0,""..tranceBuffBonus)

	player.fury = player.fury + bearBuffBonus
...
uncast = function(player) 
	...
	player.fury = player.fury - bearBuffBonus
	player:calcStat() 
	player.registry["bear_buff_bonus"] = 0
	player:sendMinitext("Your Strength of the Bear fades away")
