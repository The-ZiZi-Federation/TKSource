...
on_learned = function(player) player.registry["learned_double_step_sam"] = 1 end,
on_forget = function(player) player.registry["learned_double_step_sam"] = 0 end,
...
cast = function(player)
	...
	player:sendStatus()
	player:playSound(30)
	player:setAether("double_step_sam", 3000)
			...
			if getPass(m,x,y-1) == 0 and player:objectCanMove(x,y-1, 0) then
				if getPass(m,x,y-2) == 0 and player:objectCanMove(x,y-2, 0) then
					if getPass(m,x,y-3) == 0 and player:objectCanMove(x,y-3, 0) then
						if pc[1] ~= nil or mob[1] ~= nil or npc[1] ~= nil then double_step_sam.warp(player, m, x, y-2) return else
							double_step_sam.warp(player, m, x, y-3)
							return
						end
					return else
						double_step_sam.warp(player, m, x, y-2)
					end
				return else
					double_step_sam.warp(player, m, x, y-1)
				end
			end
		end
	elseif s == 1 then
		...
		if getPass(m,x+1,y) == 0 then	
			if getPass(m,x+2,y) == 0 then	
				if getPass(m,x+3,y) == 0 then
					if pc[1] ~= nil or mob[1] ~= nil or npc[1] ~= nil then double_step_sam.warp(player, m, x+2, y) return else
						double_step_sam.warp(player, m, x+3, y)
						return
					end
				return else	
					double_step_sam.warp(player, m, x+2, y)
				end	
			return else	
				double_step_sam.warp(player, m, x+1, y)	
			end	
		end	
	elseif s == 2 then
