...
-----------------player vs mob-------------------------------
	...
	if (target.blType == BL_MOB) then
		player.critChance = 1
		damage = ((0.025 * player.maxHealth) + (0.1 * player.level) + swingDamage(player, target, 2)) * 2.5
		...
		for i = 1, 4 do
			if (mobSideTargets[i] ~= nil) then
				player.critChance = 1
				damage = ((0.025 * player.maxHealth) + (0.1 * player.level) + swingDamage(player, mobSideTargets[i], 2)) * 2.5
