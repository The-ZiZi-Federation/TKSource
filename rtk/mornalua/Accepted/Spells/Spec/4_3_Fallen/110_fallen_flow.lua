...
on_learn = function(player) player.registry["learned_fallen_flow"] = 1 end,
on_forget = function(player) player.registry["learned_fallen_flow"] = 0 end,
		...
		if player.magic < magicCost then notEnoughMP(player) return else
			...
			if target ~= nil then
				if target.state == 1 or player.health <= 0 then return else
					...
					player:calcStat()
					player:sendStatus()
					player:sendMinitext("You cast Fallen Flow  You heal your target for "..healAmount.." Vita.")
					...
					if target.blType == BL_PC and target.ID ~= player.ID then 
						target:sendMinitext(player.name.." cast Fallen Flow on you.  You are healed for "..healAmount.." Vita.")
					end
				end
			end
		end
...
requirements = function(player)
	...
	local desc = {"Fallen Flow is a spell that can be used to heal others.", txt}
