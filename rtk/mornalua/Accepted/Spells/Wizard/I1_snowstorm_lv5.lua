...
on_learn = function(player) player.registry["learned_snowstorm_lv5"] = 1 
    ...
    on_forget = function(player) player.registry["learned_snowstorm_lv5"] = 0 end,
...
---------------------------------
	...
	player:sendMinitext("You cast Snowstorm Lv5")
	
	if player.registry["extra_spell_info"] == 1 then
		player:sendMinitext("Snowstorm Lv5 DMG: "..damage)
	end
...
----------------player vs player-----------------------------------------------------------	
	elseif (target.blType == BL_PC and player:canPK(target)) then
		...
		target.attacker = player.ID
		target:removeHealthExtend(damage, 1, 1, 0, 1, 1)
		target:sendMinitext(player.name.." burns you with Snowstorm Lv5")
		if r <= 50 then 
			if not target:hasDuration("slow") then
				target:setDuration("slow", slowDuration)
			end
		...
		for i = 1, 4 do
			if (pcSideTargets[i] ~= nil) then
				if pcSideTargets[i].state ~= 1 and player:canPK(pcSideTargets[i]) then
					...
					pcflankTargets[i]:sendMinitext(player.name.." burns you with Snowstorm Lv5.")
...
requirements = function(player)
	...
	local desc = {"Snowstorm Lv5 pelts an opponent with snow!\n\nReplaces Snowstorm lv4", txt}
