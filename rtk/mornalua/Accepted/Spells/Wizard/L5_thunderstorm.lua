...
thunderstorm = {
    ...
    on_learn = function(player) player.registry["learned_thunderstorm"] = 1 end,
    on_forget = function(player) player.registry["learned_thunderstorm"] = 0 end,
...
---------------player Targetting player------------------------------
		elseif (target.blType == BL_PC) then
			...
			player:sendMinitext("You cast Thunderstorm")
			player:playSound(sound)
			player:setAether("thunderstorm", aether)
			--- What happens to the player target on cast --------------------------------------		
		target:sendMinitext(player.name.." sends a Thunderstorm to surround you")
			target:sendAnimation(anim)
			target:playSound(sound)
			target.registry["thunderstorm"] = player.ID
			target:setDuration("thunderstorm", duration, player.ID)
		-----------------------------------------------------------------------------
		end
...
uncast = function(player)
	...
	player.registry["thunderstorm"] = 0
...
--Varable Declarations
----------------------
	local user = Player(player.registry["thunderstorm"])
...
--  Get targets around the player Thunderstorm was cast upon ------------------------------------
			...
			if distanceSquare(player, mob_around[i], 1) then
				if (player.blType == BL_PC) then
					if player.gmLevel > 0 then
						player:sendMinitext("Thunderstorm DMG: "..damage)
					end
				end
				thunderstorm.takeDamage(player, mob_around[i], damage)
			end
		end
	end
			...
			if distanceSquare(player, pc_around[i], 1) then
				if pc_around[i].ID ~= player.ID then
					if pc_around[i].state ~= 1 and player:canPK(pc_around[i]) then
						thunderstorm.takeDamage(player, pc_around[i], damage)
					end
				end
			end
		end
...
takeDamage = function(player, target, damage)
	local caster = Player(player.registry["thunderstorm"])
	local threat
	local anim = 148
	local sound
	local duration = 3000
...
requirements = function(player)
	...
	local desc = {"Summons a storm to surround you and zap nearby foes.", txt}
