...
on_learn = function(player) player.registry["learned_flare_lv2"] = 1 
    ...
    on_forget = function(player) player.registry["learned_flare_lv2"] = 0 end,
...
-----------------player vs mob-------------------------------
	if (target.blType == BL_MOB) then
		...
		if player.registry["extra_spell_info"] == 1 then
			player:sendMinitext("Flare Lv2 DMG: "..damage)
		end
...
----------------player vs player-----------------------------------------------------------	
	elseif (target.blType == BL_PC and player:canPK(target)) then
		...
		player.magic = player.magic - magicCost
		player:sendStatus()
		player:setAether("flare_lv2", aether)
		player:playSound(sound)
		target:sendAnimation(anim)
		player:sendMinitext("You cast Flare Lv2")
		target.attacker = player.ID
...
requirements = function(player)
	...
	local desc = {"Flare Lv2 sets an opponent ablaze!\n\nReplaces Flare Lv1", txt}
