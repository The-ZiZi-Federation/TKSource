...
on_learn = function(player) player.registry["learned_zap_lv4"] = 1
	player:removeSpell("zap_lv1") 
	player:removeSpell("zap_lv2") 
	player:removeSpell("zap_lv3") 
	player:removeSpell("zap_lv5") 
end,
    on_forget = function(player) player.registry["learned_zap_lv4"] = 0 end,
...
---------------------------------
	...
	player:sendMinitext("You cast Zap Lv4")
	
	if player.registry["extra_spell_info"] == 1 then
		player:sendMinitext("Zap Lv4 DMG: "..damage)
	end
...
----------------player vs player-----------------------------------------------------------	
	elseif (target.blType == BL_PC and player:canPK(target)) then
		...
		target.attacker = player.ID
		target:removeHealthExtend(damage, 1, 1, 0, 1, 1)
		target:sendMinitext(player.name.." burns you with Zap Lv4")
		if r <= 50 then 
			if not target:hasDuration("shock") then
				target:setDuration("shock", shockDuration)
			end
		...
		for i = 1, 4 do
			if (pcSideTargets[i] ~= nil) then
				if pcSideTargets[i].state ~= 1 and player:canPK(pcSideTargets[i]) then
					...
					pcflankTargets[i]:sendMinitext(player.name.." burns you with Zap Lv4.")
...
requirements = function(player)
	...
	local amounts = {4500, 12, 25}
	local txt = "In order to learn this spell, you must bring me:\n\n"
	for i = 1, #item do txt = txt..""..amounts[i].." "..Item(item[i]).name.."\n" end
	
	local desc = {"Zap Lv4 electrocutes an opponent!\n\nReplaces Zap Lv3", txt}
