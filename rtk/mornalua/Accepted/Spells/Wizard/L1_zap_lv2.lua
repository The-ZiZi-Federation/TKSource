...
zap_lv2 = {
    ...
    on_learn = function(player) player.registry["learned_zap_lv2"] = 1
	player:removeSpell("zap_lv1") 
	player:removeSpell("zap_lv3") 
	player:removeSpell("zap_lv4") 
	player:removeSpell("zap_lv5")
	end,
    on_forget = function(player) player.registry["learned_zap_lv2"] = 0 end,
...
-----------------player vs mob-------------------------------
	if (target.blType == BL_MOB) then
		...
		player:sendMinitext("You cast Zap Lv2")
		
		if player.registry["extra_spell_info"] == 1 then
			player:sendMinitext("Zap Lv2 DMG: "..damage)
		end
...
----------------player vs player-----------------------------------------------------------	
	elseif (target.blType == BL_PC and player:canPK(target)) then
		...
		target.attacker = player.ID
		target:removeHealthExtend(damage, 1, 1, 0, 1, 1)
		target:sendMinitext(player.name.." burns you with Zap Lv2")
		if r <= 50 then 
			if not target:hasDuration("shock") then
				target:setDuration("shock", shockDuration)
			end
...
requirements = function(player)
	...
	local level = 30
	local item = {0, 388, 50}
	local amounts = {5500, 25, 25}
	local txt = "In order to learn this spell, you must bring me:\n\n"
	for i = 1, #item do txt = txt..""..amounts[i].." "..Item(item[i]).name.."\n" end
	
	local desc = {"Zap Lv2 electrocutes an opponent!\n\nReplaces Zap Lv1", txt}
