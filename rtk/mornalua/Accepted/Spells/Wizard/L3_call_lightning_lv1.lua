...
call_lightning_lv1 = {
    ...
    on_learn = function(player) player.registry["learned_call_lightning_lv1"] = 1 player:removeSpell("call_lightning_lv2") end,
    on_forget = function(player) player.registry["learned_call_lightning_lv1"] = 0 end,
...
---------------------------
	...
	player:sendStatus()
	player:setAether("call_lightning_lv1", aether)
	player:playSound(sound)
	player:sendMinitext("You cast Call Lightning Lv1.")
	if player.registry["extra_spell_info"] == 1 then
		player:sendMinitext("Call Lightning Lv1 DMG: "..damage)
	end
	...
	elseif (target.blType == BL_PC) then
		...
		if player:canPK(target) then
			target:removeHealthExtend(damage, 1, 1, 0, 1, 1)
			target:sendMinitext(player.name.." zaps you with Call Lightning Lv1.")
			if r <= 50 then 
				if not target:hasDuration("shock") then
					target:setDuration("shock", duration)
				end
	...
	if (#targets > 0) then
		for i = 1, #targets do
			...
			elseif (targets[i] ~= nil and targets[i].blType == BL_PC and player:canPK(targets[i])) then
				targets[i].attacker = player.ID
				targets[i]:removeHealthExtend(damage, 1, 1, 0, 1, 1)
				targets[i]:sendMinitext(player.name.." zaps you with Call Lightning Lv1.")
				if r <= 50 then 
					if not targets[i]:hasDuration("shock") then
						targets[i]:setDuration("shock", duration)
					end
...
requirements = function(player)
	...
	local desc = {"Call Lightning Lv1 will drop a lightning bolt on an area!", txt}
