...
--   Last Edited: 07/07/17
-------------------------------------------------------

fireball_lv1 = {

    on_learn = function(player) player.registry["learned_fireball_lv1"] = 1 player:removeSpell("fireball_lv2") end,
    on_forget = function(player) player.registry["learned_fireball_lv1"] = 0 end,
...
---------------------------
	...
	player:sendStatus()
	player:setAether("fireball_lv1", aether)
	player:playSound(sound)
	player:sendMinitext("You cast Fireball Lv1.")
	if player.registry["extra_spell_info"] == 1 then
		player:sendMinitext("Fireball Lv1 DMG: "..damage)
	end
	...
	elseif (target.blType == BL_PC) then
		...
		if player:canPK(target) then
			target:removeHealthExtend(damage, 1, 1, 1, 1, 1)
			target:sendMinitext(player.name.." burns you with a Fireball Lv1.")
			if r <= 50 then 
				if not target:hasDuration("seared") then
					target:setDuration("seared", duration)
				end
	...
	if (#targets > 0) then
		for i = 1, #targets do
			...
			elseif (targets[i] ~= nil and targets[i].blType == BL_PC and player:canPK(targets[i])) then
				damage = damage / 50
				targets[i].attacker = player.ID
				targets[i]:removeHealthExtend(damage, 1, 1, 1, 1, 1)
				targets[i]:sendMinitext(player.name.." burns you with a Fireball Lv1.")
				if r <= 50 then 
					if not targets[i]:hasDuration("seared") then
						targets[i]:setDuration("seared", duration)
					end
...
requirements = function(player)
	...
	local desc = {"Fireball Lv1 will drop a burning sphere on an area!", txt}
