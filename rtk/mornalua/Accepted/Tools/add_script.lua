...
click = async(function(player, npc)
	...
	if red > 0 then red_mob = Mob(red).name else red_mob = "None" end
	if blue > 0 then blue_mob = Mob(blue).name else blue_mob = "None" end
	if green > 0 then green_mob = Mob(green).name else green_mob = "None" end
	if yellow > 0 then yellow_mob = Mob(yellow).name else yellow_mob = "None" end
	if pink > 0 then pink_mob = Mob(pink).name else pink_mob = "None" end
	local opts = {}
...
clone = function(player, npc)
	...
	if coat ~= nil then npc.gfxArmor = coat.look npc.gfxArmorC = coat.lookC else
		if armor ~= nil then npc.gfxArmor = armor.look npc.gfxArmorC = armor.lookC else npc.gfxArmor = player.sex end
	end
	return npc
end
}
