...
--	player:refresh()
	...
	if player.ID == 2 or player.ID == 4 or player.ID == 7 then table.insert(a, "Extended Tools") end
	...
	elseif menu == "Approve Clan Creation on Player" then
		...
		player:dialogSeq({t,"This tool sets the proper registry values for a given player so that they may begin the Clan Trials process with their respective group.\n\nInform player that they will need to be the group leader in every trial and will become the defacto Primogen."},1)
		...
		local confirm = player:menuString("Are these options correct?\n\nSacrifice: "..sacrifice.."\nGood vs. Evil: "..goodvsevil.."\nStorytelling: "..storytelling.."\nWealth: "..wealth.."\nEndurance: "..endurance,{"Yes","No"})
...
------- Convert string option to necessary registry values -----
		...
		else Player(inputPlayer).registry["clan_trial_sacrifice"] = 0
		end

		if goodvsevil == "Yes" then
			Player(inputPlayer).registry["clan_trial_goodvsevil"] = 1
		else Player(inputPlayer).registry["clan_trial_goodvsevil"] = 0
		end
		
		if storytelling == "Yes" then
			Player(inputPlayer).registry["clan_trial_storytelling"] = 1
		else Player(inputPlayer).registry["clan_trial_storytelling"] = 0
		end

		if wealth == "Yes" then
			Player(inputPlayer).registry["clan_trial_wealth"] = 1
		else Player(inputPlayer).registry["clan_trial_wealth"] = 0
		end
		...
		Player(inputPlayer):dialogSeq({t, "Your clan request has been approved. Please visit the Clan NPC in Hon for more information."})
