...
debug_exp = function(player, level)
	player:talkSelf(0,"Total Exp required to level to "..level.." is "..format_number(totalExp(level)))
	player:talkSelf(0,"Sells to level at "..level.." is "..format_number(sellCount(level)))
	player:talkSelf(0,"Cost of stat at level "..level.." is "..format_number(expCostAtLevel(level)))
	player:talkSelf(0,"Experience required to level from "..level.." to "..(level+1).." is "..format_number(toNextLevel(level)))
	player:talkSelf(0,"Cumulative stats on you at start of "..level..format_number(totalStatAtLevel(level)))
end

debug_clock = function()

	local dt = mynexia_lore_time()
	...
	broadcast(-1, "mYnexia "..dt.Year.." |  "..dt.SeasonOfYear..", "..dt.DayOfWeek..", "..dt.MonthOfYear.." "..dt.DayOfMonth.." - "..dt.Time.." o' clock")
end