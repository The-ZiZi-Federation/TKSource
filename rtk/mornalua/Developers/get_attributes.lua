	...
	--[[ Get Info ]]-----------------------------------------------------------------------------------
	--> iconViewer        							--> getDirection(block1, block2)
	--> iconViewer2                                 --> getAngle(block1, block2)
	--> iconViewer3                                 --> getAngleXY(block, targetX, targetY)
	--> monsterViewer	                            --> objBrowser()
	--> monsterViewer2                              --> tileBrowser()
	--> monsterViewer3                              --> getSide(block, target)
	--> getTargetsAround(block, blockType)			--> canMove(block)
	--> getTargetXY(m, x, y, bltype, priority)		--> getExp(player, limit, ratio)
	--> getTileFacing(block, side)                  --> getHalfSide(block)
	--> getObjFacing(block, side)                   --> checktile(tipo, dist)    
	--> getPassFacing(block, side)                  --> checkTileType(tipo, dist)
	--> checkMove(block, m, x, y, side)             --> getPushBackCoor(player, numoftile)
	--> distance(block, target)                     --> distanceSquareXY(block, targetX, targetY, dist)
	--> distanceXY(block, x, y)                     --> findObjects(block, maxDist)                    
	--> distanceSquare(block, target, dist)         --> getTargetFacing(block, bltype, priority, dist) 
...
getObjFacingXY = function(block, side)
	local nside
	if (side ~= nil) then
		nside = side
	else
...
getLeftObjFacing = function(block, side)
	local nside
	if (side ~= nil) then
		nside = side
	else
...
getRightObjFacing = function(block, side)
	local nside
	if (side ~= nil) then
		nside = side
	else
...
curT = function()
	timemark = "(Moon "..curYear()..", "..getCurSeason()..")"
	return timemark
end
...
getExp = function(player, limit, ratio)
	...
	if (ratio == nil) then ratio = 75 end
	ratio = ratio / 100
	
	if (limit == nil) then limit = 1250 end
	if (level > limit) then level = limit end
...
monsterViewer = async(function(player, str)
	if (str == nil) then
		str = "n"
	end
	
	local x = player.registry["gfx_monsters"]
	local y = player.registry["gfx_monsters_color"]
	local t = {graphic = convertGraphic(x, "monster"), color = y}
	player.npcGraphic = t.graphic
	player.npcColor = t.color
	player:dialogSeq({t, "Monster #: "..x.."\nColor #: "..y}, 1)
	if (str == "n") then
		player.registry["gfx_monsters"] = player.registry["gfx_monsters"] + 1
		monsterViewer2(player, "n")
	elseif (str == "p") then
		player.registry["gfx_monsters"] = player.registry["gfx_monsters"] - 1
		monsterViewer2(player, "p")
	elseif (str == "nc") then
		player.registry["gfx_monsters_color"] = player.registry["gfx_monsters_color"] + 1
		monsterViewer2(player, "nc")
	elseif (str == "pc") then
		player.registry["gfx_monsters_color"] = player.registry["gfx_monsters_color"] - 1
		monsterViewer2(player, "pc")
	end
end)

monsterViewer2 = function(player, str)
	if (str == nil) then
		str = "n"
	end
	...
	if (str == "n") then
		player.registry["gfx_monsters"] = player.registry["gfx_monsters"] + 1
		monsterViewer3(player, "n")
	elseif (str == "p") then
		player.registry["gfx_monsters"] = player.registry["gfx_monsters"] - 1
		monsterViewer3(player, "p")
	elseif (str == "nc") then
		player.registry["gfx_monsters_color"] = player.registry["gfx_monsters_color"] + 1
		monsterViewer3(player, "nc")
	elseif (str == "pc") then
		player.registry["gfx_monsters_color"] = player.registry["gfx_monsters_color"] - 1
		monsterViewer3(player, "pc")
	end
end

monsterViewer3 = function(player, str)
	if (str == nil) then
		str = "n"
	end
...
getPushBackCoor = function(player, numoftile)
    local coor = { m = 0, x = 0, y = 0}
    coor.m = player.m
    coor.x = player.x
    coor.y = player.y
    
    if player.side == 0 then 
        coor.y = player.y + numoftile
    end
    if player.side == 1 then 
        coor.x = player.x - numoftile
    end
