...
canRunLuaTalk = function(player) checkLuaSafety(player, player.speech) end
canRunLuaMail = function(player) checkLuaSafety(player, player.mail) end
...
checkLuaSafety = function(player, check)
	...
	repeat strCheck = string.find(check, "Player", strCheck + 1)
		if (strCheck ~= nil) then checkCount = checkCount + 1 end
	until (strCheck == nil)
	
	if (player:staff("script") == 5) then player.luaExec = 1 return end

	--if (player:staff("script") >= 1)
	for i = 1, #id do
		if player.ID == id[i] then
			if checkCount == 0 then player.luaExec = 1 else 
				strCheck = 0
				repeat
					...
					if (string.byte(check, strCheck + 7) == 34) then
						...
						if (strCheck == nil) then player.luaExec = 0 else player.luaExec = 1 end
