...
local players = core:getObjectsInMap(m, BL_PC)
	...
	if (core.gameRegistry["bomber_war_playing"] == 1) then
		...
		if (core.gameRegistry["bomber_war_wall_timer"] == 55 or core.gameRegistry["bomber_war_wall_timer"] == 115 or core.gameRegistry["bomber_war_wall_timer"] == 175 or core.gameRegistry["bomber_war_wall_timer"] == 235 or core.gameRegistry["bomber_war_wall_timer"] == 295) then
			broadcast(15050, "-----------------------------------------------------------------------------------------------------")
			broadcast(15050,                                          "WARNING - Play Area Reduction in 5 Seconds. Get Ready!")
			broadcast(15050, "-----------------------------------------------------------------------------------------------------")
		end
	
		if (core.gameRegistry["bomber_war_wall_timer"] == 60) then -- First set outside wall x 4
			for x = 1, 34 do   --- horizontal walls
				for y = 4, 5 do
					bomber_war.drawWall(x, y)
				end
				for y = 36, 37 do
					bomber_war.drawWall(x, y)
				end
		...
		elseif (core.gameRegistry["bomber_war_wall_timer"] == 120) then -- second outside wall x 4
			for x = 3, 32 do
				for y = 6, 7 do
					bomber_war.drawWall(x, y)
				end
		...
		elseif (core.gameRegistry["bomber_war_wall_timer"] == 180) then -- third outside wall x 4
			for x = 5, 30 do    --- horizontal walls
				for y = 8, 9 do
					bomber_war.drawWall(x, y)
				end
				for y = 32, 33 do
		...
		elseif (core.gameRegistry["bomber_war_wall_timer"] == 240) then -- fourth outside wall x 4
			for x = 7, 28 do    --- horizontal walls
				for y = 10, 11 do
					bomber_war.drawWall(x, y)
				end
				for y = 30, 31 do
		...
		elseif (core.gameRegistry["bomber_war_wall_timer"] == 300) then -- fifth outside wall x 4
			for x = 9, 26 do    --- horizontal walls
				for y = 12, 13 do
					bomber_war.drawWall(x, y)
				end
				for y = 28, 29 do
...
--Player(4):talk(0,"b: "..#livingBlue)
	...
	if core.gameRegistry["bomber_war_playing"] == 1 then
		...
		elseif #livingRed > 0 and #livingBlue == 0 then 
			...
			return bomber_war.redWin() 
	
		elseif #livingRed == 0 and #livingBlue > 0 then 
			core.gameRegistry["bomber_war_winner"] = 2
			return bomber_war.blueWin() 
...
getStartTimer = function()
	...
	if core.gameRegistry["bomber_war_start"] < os.time() then return "00:00:00" else
...
click = async(function(player, npc)
	...
	local str, par = "", ""
	local time = bomber_war.getStartTimer()
	...
	if menu == "How To Play?" then
		player:dialogSeq({t, n.."Bomber War is an explosive team-based game!", 
							...
							n.."If you're caught in an explosion, you're dead. The team with a living player is the winner."}, 1)
		player:freeAsync()
		bomber_war.click(player, npc)
	elseif menu == "Register For Bomber War" then
		...
		if player.registry["bomber_war_team"]  == 0 then
			...
			player:playSound(29)
			player:dialogSeq({t, n.."Allright, your character is registered for Bomber War.\nPlease wait until the game starts!"}, 1)
		else
...
core = function()
	...
	bomber_war.closed()
	bomber_war.begin(core)
	bomber_war.balancing(core)
	bomber_war.endGame()
end,
...
open = function()
	...
	core.gameRegistry["bomber_war_start"] = os.time()+300
	broadcast(-1, "-----------------------------------------------------------------------------------------------------")
...
entryLegend = function(player)
	...
	if player:hasLegend("bomber_war_entries") then player:removeLegendbyName("bomber_war_entries") end
	
	if reg > 0 then
		player.registry["bomber_war_entries"] = player.registry["bomber_war_entries"] + 1
		player:addLegend("Played in "..player.registry["bomber_war_entries"].." Bomber Wars", "bomber_war_entries", 76, 16)
	else
...
victoryLegend = function(player)
	...
	if player:hasLegend("bomber_war_wins") then player:removeLegendbyName("bomber_war_wins") end
	...
	if core.gameRegistry["bomber_war_players"] >= 2 then
		if #pc > 0 then
			for i = 1, #pc do
				...
				bomber_war.entryLegend(pc[i])
				bomber_war.costume(pc[i])
				bomber_war.enterArena(pc[i])
			end
			bomber_war.wait()
		end
	else
		...
		if getPass(map,x,y) == 0 then
			if getObject(map,x,y) == 0 then
				if #pc == 0 then
					...
					return bomber_war.enterArena(player)
				end
			else	
				return bomber_war.enterArena(player)
			end
...
balancing = function(npc)
	...
	if #pc > 0 then
		for i = 1, #pc do	
			if #red > #blue then
				if (#red-#blue) ~= 1 then pc[math.random(#pc)].registry["bomber_war_team"] = 2 break end
			end
			if #red < #blue then
				if #blue - #red ~= 1 then pc[math.random(#pc)].registry["bomber_war_team"] = 1 break end
			end
		end
	end
end,
...
bomb = function(player)
	...
	if core.gameRegistry["bomber_war_playing"] == 0 then player:sendMinitext("The game isn't playing now!") return end
	
	if #tileBombs > 0 then player:sendMinitext("There is already a bomb here!") return end
	...
	if m == 15050 then
		if player.registry["bomber_war_registered"] > 0 then
			if #ownBomb < maxBombs then
				...
				player:dropItemXY(21001, 1, m, x, y)
				player:addNPC("bomber_war_bomb", m, x, y, 1000, 3000, player.ID)
			else
...
mapReset = function()
	...
	for x = 1, 34 do
		for y = 4, 37 do
			if getTile(15050,x,y) == 31154 or getTile(15050,x,y) == 31123 or getTile(15050,x,y) == 29745 or getTile(15050, x, y) == 29259 then
