...
cast = function(player)
	...
	for i = 1, 7 do
		...
		elseif pc ~= nil then
			...
			if player:canPK(pc) then pc:removeHealthExtend(d,1,1,1,1,0) end
			return
		end
					
		if s == 0 then
			if getPass(m,x,y-i) > 0 then player:sendAnimationXY(424,x,y-i) player:playSound(347) player:playSound(348) return end
		elseif s == 1 then
			if getPass(m,x+i,y) > 0 then player:sendAnimationXY(424,x+i,y) player:playSound(347) player:playSound(348) return end
		elseif s == 2 then
			if getPass(m,x,y+i) > 0 then player:sendAnimationXY(424,x,y+i) player:playSound(347) player:playSound(348) return end
		elseif s == 3 then
			if getPass(m,x-i,y) > 0 then player:sendAnimationXY(424,x-i,y) player:playSound(347) player:playSound(348) return end
