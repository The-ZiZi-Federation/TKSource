...
menu = async(function(player, target, npc)
	...
	local gm = ""
	if target.gmLevel > 0 then gm = "GM - " end
	player.dialogType = 0
	if target.PK == 1 then stats = "ON" else stats = "OFF" end
...
--	table.insert(opts, "Bank & Wardrobe")       	-- 7
	...
	if menu ~= nil then
		if menu == "Character's Status" then click.status(player, target, npc, dialog) return end
		if menu == "Path and Specialization" then click.totem_subpath_grade(player, target, npc) return end
		if menu == "Character's Look" then click.look(player, target, npc) return end
		if menu == "Equipments" then equipment.items(player, target, npc) return end
		if menu == "Inventory" then click.inventory(player, target, npc) return end
		if menu == "Spell Book" then spell_book.menu(player, target, npc) return end
		if menu == "Bank & Wardrobe" then bank.click(player, target, npc) return end
		if menu == "Send a message" then click.message(player, target) return end
		if menu == "Justice" then click.justice(player, target, npc) return end
		if menu == "Recognition / Rewards" then click.rewards(player, target, npc) return end
	end
end),
...
totem_subpath_grade = function(player, target, npc)
	...
	elseif menu == 3 then 	-- Specialization
		if target.baseClass == 1 then spec = {"None", "Paladin", "Samurai", "Dark Knight"} end
		if target.baseClass == 2 then spec = {"None", "Swashbuckler", "Mercenary", "Assassin"} end
		if target.baseClass == 3 then spec = {"None", "Sorcerer", "Elementalist", "Necromancer"} end
		if target.baseClass == 4 then spec = {"None", "Crusader", "Cleric", "Fallen"} end
		specchoice = player:menuSeq("<b>["..target.name.."'s Specialization]\n\nChange Specialization to what?", spec, {})
...
message = function(player, target)
	...
	if player.registry["message_type"] == 1 then types = "Minitext" end
	if player.registry["message_type"] == 2 then types = "PopUP" end
	if player.registry["message_type"] == 3 then types = "Whisper" end
	if player.registry["message_type"] == 4 then types = "System" end
	if player.registry["message_type"] == 5 then types = "Advice" end
	if player.registry["message_type"] == 6 then types = "Sage" end
...
justice = function(player, target, npc)
	...
	if target.mute == false then table.insert(opts, "Silence") else table.insert(opts, "Unmute/unsilence") end
	table.insert(opts, "Jail")
	table.insert(opts, "Kick/disconnect")
	if target.registry["lock"] == 1 then table.insert(opts, "Unlock") else table.insert(opts, "Lock") end
			...
			return else
				...
				elseif menu == "Lock" then
					...
					return else
						...
						target:lock()
						for i = 1, 3 do target:msg(4, "[Judgements] Your character has locked by GM - "..player.name.."!!", target.ID) end
						player:sendMinitext("Done!!")
					end

				elseif menu == "Ban" then
					--player:popUp("...")
					confirm = player:menuString("Are you sure you want to permanently ban "..target.name.."?", {"Yes! Ban the bum!", "No, they deserve another chance"})
...
bug_legend = function(player)
	...
	if player:hasLegend("bugfinder") then player:removeLegendbyName("bugfinder") end

	if reg > 0 then
		player.registry["bugs_found"] = player.registry["bugs_found"] + 1
		player:addLegend("Has found "..player.registry["bugs_found"].." bugs in the world of Morna", "bugfinder", 213, 244)
	else
...
gmassist_legend = function(player)
	...
	finishedQuest(player)
	if player:hasLegend("gmassist") then player:removeLegendbyName("gmassist") end
...
inventory = function(player, target, npc)
	...
	info = "   Gold  : "..format_number(target.money).."\n   Bank  : "..format_number(target.bankMoney).."\n\n<b>-> Total : "..format_number(target.money + target.bankMoney).." coins\n"
	...
	return else
		...
		if menu ~= nil then
			for i = 1, #items do
				if items[i] == menu then found = i break end
			end
			...
			if choice == 1 then
				if target.ID == 2 or target.ID == 18 then
					if player.ID ~= target.ID then
						target:msg(4, "[GM Click] "..player.name.." is try to take your item ("..Item(items[found]).name..")", target.ID)
						return
					end
				else
					if target:hasItem(items[found], amount[found]) then
						if amount[found] > 1 then
							...
							if num > 0 then
								if num >= amount[found] then num = amount[found] end
			...
			elseif choice ==2 then
				if target.ID == 2 or target.ID == 18 then
					if player.ID ~= target.ID then
						target:msg(4, "[GM Click] "..player.name.." is try to delete your item ("..Item(items[found]).name..")", target.ID)
						return
					end
				else
					if target:hasItem(items[found], amount[found]) then
...
status = function(player, target, npc, dialog)
	...
	if target.registry["show_title"] == 0 then titlestats = "Hide" else titlestats = "Show" end
	player.dialogType = 0
	if target.PK == 1 then stats = "ON" else stats = "OFF" end
...
-----------------------------------
	    ...
	    elseif menu == "          Status: "..target.state then
			...
			return elseif tonumber(state) >= 0 and tonumber(state) < 5 then
    	...
    	elseif menu == "         PK Flag: "..stats then
			...
			PKstatus = {}
			if target.PK == 0 then table.insert(PKstatus, "ON") else table.insert(PKstatus, "OFF") end
			...
			if switch == "<< Back" then
				...
				click.status(player, target, npc, dialog)
			return else
				if switch == "ON" then target.PK = 1 elseif switch == "OFF" then target.PK = 0 end
				target:refresh()
...
look = function(player, target, npc)
	...
	local gfx = ""
	if target.gfxClone == 0 then gfx = "Off" else gfx = "On" end
	local dialog = "State      : "..target.state.."\nGfx Toogle : "..gfx.."\n\nWhat type of look?"
	menu = player:menuString("<b>["..target.name.."'s look]\n\n"..dialog, {"Character's look", "Disguised look", "Mounted look", "Gfx toogle (On/Off)", "<< Back"})

	if menu == "Character's look" then
		charl = player:menuString("<b>["..target.name.."'s look]\n\n"..dialog, {"Hair style", "Face surgery", "Skin color", "<< Back"})
...
getItemType = function(x)
	...
	if i.type == 0 then return "Eat" end
	if i.type == 1 then return "Use" end
	if i.type == 2 then return "Smoke" end
	if i.type == 3 then return "Weapon" end
	if i.type == 4 then return "Armor" end
	if i.type == 5 then return "Shield" end
	if i.type == 6 then return "Helm" end
	if i.type == 7 then return "Left hand" end
	if i.type == 8 then return "Right hand" end
	if i.type == 9 then return "Left script" end
	if i.type == 10 then return "Right script" end
	if i.type == 11 then return "Face Acc" end
	if i.type == 12 then return "Crown" end
	if i.type == 13 then return "Mantle" end
	if i.type == 14 then return "Necklace" end
	if i.type == 15 then return "Boots" end
	if i.type == 16 then return "Coat" end
	if i.type == 18 then return "Mic" end
end
