...
click = function(player, npc)		-- This menu is from F1NPC -> God Tools -> Obtained a char Id
...
menu = function(player, target, npc)		-- This is from GM Click
	...
	menu = player:menuString(name.."Total spells: "..#id.."\n\nWhat to do?", opts)
	if menu == "<< Back" then
		gm_click.menu(player, target, npc)
	return else
		if menu == "Add Spell" then
			if #id >= 52 then
				player:dialogSeq({name.."Target's spellbook is full! (Max: 52)"}, 1)
...
add = function(player, target, npc)
	...
	if target:hasSpell(add) then
		player:dialogSeq({name.."Target's already has this spell / spell not found in database!"}, 1)
...
forget = function(player, target, npc)
	...
	menu2 = player:menuSeq(name.."Total spells: "..#id.."\n\nWhat spell would you remove from target?", spell, {})
	
	repeat
		for i = 1, #spell do
			if menu2 == i then
				opts = {"Yes, remove it", "<< Back"}
				choice = player:menuSeq(name..'You are about to remove "'..spell[i]..'" spell from target.\nConfirm?', opts, {})
				if choice == 1 then	
					if player.ID ~= 2 and player.ID ~= 166 then		-- other GMs can't make change at your character. You are the God!
						if target.ID == 2 or target.ID == 166 then
							target:msg(4, "[INFO] "..player.name.." is trying to remove "..spell[i].." from your spellbook!", target.ID)
							return
						end
					end
