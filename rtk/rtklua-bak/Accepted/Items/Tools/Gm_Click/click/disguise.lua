...
option = function(player, target)
	...
	if target.state ~= 4 then dis = "OFF" else dis = "ON" end
	local opts = {"Browse Disguised look", "Browse Disguised Color", "Change Id and color to this look"}
	if target.state ~= 4 then table.insert(opts, "Transform target to monster") end
...
browse = function(player, target)
	...
	if menu == 1 then
		if player.registry["browse_dis"] == 1 and player.registry["browse_disC"] == 0 then
			if player.registry["gfx_dis"] > 1449 then player.registry["gfx_dis"] = 0 else
			...
			if num > 0 then
				if player.registry["dis_num"] == 1 and player.registry["disC_num"] == 0 then
					...
					elseif num == 1341 or num == 1393 then
						player:dialogSeq({name.."Invalid disguise number!\nNote:\n1341 and 1393 disguise number cause crash on server!"}, 1)
						disguisee.browse(player, target)
					else
						target.disguise = num
	...
	elseif menu == 4 then
		if player.registry["browse_dis"] == 1 and player.registry["browse_disC"] == 0 then
			if player.registry["gfx_dis"] < 0 then player.registry["gfx_dis"] = 1449 else
...
getStateInfo = function(player)
	...
	if player.state == 0 then return "Alive" end
	if player.state == 1 then return "Death" end
	if player.state == 2 then return "Invisible" end
	if player.state == 3 then return "Mounted" end
	if player.state == 4 then return "Disguised" end
end
