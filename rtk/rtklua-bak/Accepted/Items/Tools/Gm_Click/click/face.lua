...
browse = function(player, target, npc)
	...
	elseif menu == 3 then
		if player.registry["face_style"] == 1 and player.registry["face_color"] == 0 then
			if npc.gfxFace < 200 then npc.gfxFace = 238 else
				npc.gfxFace = npc.gfxFace - 1
			end
		elseif player.registry["face_style"] == 0 and player.registry["face_color"] == 1 then
			if npc.gfxFaceC < 0 then npc.gfxFaceC = 255 else
				npc.gfxFaceC = npc.gfxFaceC - 1
			end
		end
		face.browse(player, target, npc, dialog)
...
confirmation = function(player, target, npc, dialog)
	...
	menu = player:menuSeq(dialog.."Are you sure to change "..target.name.."'s face look and eye color to this?", opts, {})
