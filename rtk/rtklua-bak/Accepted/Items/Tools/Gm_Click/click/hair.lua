...
option = function(player, target, npc)
	...
	player.dialogType = 2
	local opts = {"Browse hair style", "Browse hair color", "Change hair and color to this look", "<< Back"}
	...
	if not player.ID == 2 or not player.ID == 4 then
		if target.ID == 2 or target.ID == 4 then
			target:msg(4, "[Hair style] "..player.name.." is try to change your hair style!", target.ID)
			return
		end
	end
	...
	elseif option == 3 then
		if player.ID ~= 2 and target.ID == 2 then
			target:msg(4, "[INFO] "..player.name.." is trying to change your hair look", target.ID)
			return
		end
