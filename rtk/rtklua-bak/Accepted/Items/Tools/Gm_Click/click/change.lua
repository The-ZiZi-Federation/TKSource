...
status = function(player, target, npc, type)
	...
	return false else
		...
		if player.gmLevel > 0 then location = target.mapTitle.."\n"..target.m..", "..target.x..", "..target.y end
		if target.className == target:baseClassName(target.baseClass) then subpath = "None" else subpath = target.className end
		if target.spouse > 0 then spouse = Player(target.spouse).name else spouse = "None" end
		...
		for i = 1, #number do
			if type == number[i] then change.choice(player, target, npc, type) return end
		end	
		...
		if type == "title" then
			...
			if titleM == "Title  : "..target.title then
				...
				return else
					confirm = player:menuString("<b>["..target.name.."'s Title]\n\nAre you sure to change "..target.name.."'s title to:\n'"..title.."' ?", {"Yes", "No"})
					if confirm == "Yes" then
						target.title = title
						target:sendStatus()
						target:sendMinitext("Your title has changed!")
		...
		if type == "country" then
			menu = player:menuString("<b>["..target.name.."'s Country]\n\nCountry : "..target:countryName(target.country).."\n\nChange "..target.name.."'s country to?", {"Buya", "Kugnae"})
			if menu ~= nil then
				if menu == "Buya" then
					target.country = 2
		...
		if type == "clan" then
			local clan
			if target.clan > 0 then clan = target.clanName else clan = "None" end
			local clanC = {}
			if target.clan > 0 and target.clanName ~= nil then table.insert(clanC, "Clan info") end
...
choice = function(player, target, npc, type)
	...
	return false else
		...
		if type == "level" then value = target.level end
		if type == "vita" then value = target.baseHealth end
		if type == "mana" then value = target.baseMagic end
		if type == "might" then value = target.basemight end
		if type == "will" then value = target.basewill end
		if type == "grace" then value = target.basegrace end
		if type == "armor" then value = target.basearmor end
		if type == "exp" then value = target.exp end
		--if type == "lapis" then value = target.pp end
		if type == "gold" then value = target.money end
		if type == "speed" then value = target.speed end
		if type == "fury" then value = target.fury end
		...
		how = player:menuSeq(dialog..""..type..": "..format_number(value).."\n\nHow do you to want change "..target.name.."'s "..type.."?", opts, {})
		if how == 1 then
			increase = tonumber(math.floor(player:input(dialog.."Current : "..format_number(value).."\n\nHow much of "..type.." should be added to "..target.name.."?")))
			if value + increase > 4294967295 then
				anim(player)
				player:sendMinitext("Error value!")
				click.status(player, target, npc, dialog)
			return elseif type == "level" and value + increase > 99 then
				anim(player)
				player:sendMinitext("Error value!")
				click.status(player, target, npc, dialog)
			return elseif increase > 0 then
				confirm = player:menuString(dialog.."Before   : "..format_number(value).."\nIncrease : "..format_number(increase).."\n------------------ +\nAfter >> "..format_number(value+increase).."\nConfirm your choice", confirmC)
		...
		elseif how == 2 then
			decrease = tonumber(math.floor(player:input(dialog.."Current : "..format_number(value).."\nHow much of "..type.." should be decreased from "..target.name.."?")))
		...
		elseif how == 3 then
			input = tonumber(math.floor(player:input(dialog.."Current : "..format_number(value).."\n\nChange "..target.name.."'s "..type.." to: ")))
			if input >= 0 then
...
historyWrite = function(player, target, stats, value, type)
	...
	for i = 1, #class do
		if stats == class[i] then x = i break end
	end
	if x == 0 then return nil else
		target.registry[stats.."_before"] = change[x]
		if type == "add" then
			if stats == "level" or stats == "speed" or stats == "gmLevel" then
				if change[x]+value > 99 then
					player:msg(4, "[Error] "..player.name.." -> Target: "..target.name.." (Max "..stats..": 99) "..os.date(), player.ID)
				...
				if change[x]+value > core.ID then
					player:msg(4, "[Error] "..player.name.." -> Target: "..target.name.." (Max "..stats..": "..core.ID..") "..os.date(), player.ID)
		...
		elseif type == "remove" then
			if stats == "level" or stats == "vita" or stats == "mana" then
				if change[x]-value < 0 then
					player:msg(4, "[Error] "..player.name.." -> Target: "..target.name.." (Min "..stats..": 1) "..os.date(), player.ID)
		...
		if stats == "level" then target.level = change[x] end
		if stats == "vita" then target.baseHealth = change[x] end
		if stats == "mana" then target.baseMagic = change[x] end
		if stats == "might" then target.basemight = change[x] end
		if stats == "will" then target.basewill = change[x] end
		if stats == "grace" then target.basegrace = change[x] end
		if stats == "armor" then target.basearmor = change[x] end
		if stats == "exp" then target.exp = change[x] end
		if stats == "gold" then target.money = change[x] end
		if stats == "speed" then target.speed = change[x] end
		if stats == "PP" then target.pp = change[x] end
		if stats == "EP" then target.ep = change[x] end
		if stats == "FP" then target.fp = change[x] end
		if stats == "BP" then target.bp = change[x] end
		if stats == "gmLevel" and player.ID == 2 then target.gmLevel = change[x] end
		...
		text = text..""..string.upper(stats).." Before  : "..format_number(target.registry[stats.."_before"]).."\n"
		...
		if Player(2) ~= nil then god = Player(2)
			god:msg(4, "[Status Edit] "..player.name.." just change "..target.name.."'s "..stats.."!", god.ID)
			god:msg(4, target.name.."'s "..stats.." before("..format_number(target.registry[stats.."_before"])..") ~> ("..format_number(change[x])..")", god.ID)
		end
