insert_warps = {
	cast = function(player, question)
		local path = "../rtkmaps/warps2.txt"

		--	local path = "/root/RTK/rtkmaps/warps.txt"
		--	local path = "/root/RTK/rtkmaps/end_warps.txt"
		...
		if string.match(string.lower(player.question), "f") ~= nil or tonumber(player.question) == 1 then
			...
			player:msg(
				0,
				"From -> " ..
					player.mapTitle .. "(" .. player.m .. "),   X: " .. player.x .. ",   Y: " .. player.y .. "  >> " .. path,
				player.ID
			)
	...
	confirm = async(
		function(player, fm, fx, fy, tm, tx, ty)
			local path = "../rtkmaps/warps2.txt"
			local file = io.open(path, "a+")
	...
	startline = async(
		function(player)
			...
			--player:sendAnimationXY(228, tx, ty, 1)
			menu = player:menuSeq(name .. "" .. player.mapTitle .. "\n\nInsert this map title to the warps file?", opts, {})

			if menu == 1 then
				file:write("// // " .. player.mapTitle .. "\n")
				file:flush()
				player:playSound(6)
				player:msg(0, "Added to warps file -> // // " .. player.mapTitle, player.ID)
			end
		end
	),
	endline = async(
		function(player)
			...
			--player:sendAnimationXY(228, tx, ty, 1)
			menu = player:menuSeq(name .. "" .. player.mapTitle .. "\n\nInsert a blank line to the warps file?", opts, {})
