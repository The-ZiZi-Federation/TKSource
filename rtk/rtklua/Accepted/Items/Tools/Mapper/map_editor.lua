...
--	txt = txt.."MapFile: "..getMapFolder(player.m)..""
	...
	elseif menu == "Save Map" then
		...
		if confirm == "Yes" then
			saveMap(player.m, "../mornamaps/Accepted/"..player.mapfile)
			player:sendMinitext("Map saved!")
			gm = player:getUsers()
			for i = 1, #gm do
				if gm[i].gmLevel > 0 then
					gm[i]:msg(4, player.name.." has made changes to "..player.mapTitle.." map (Id: "..player.m..") and saved the map file!", gm[i].ID)
...
enter = function(player)
	...
	local add = {"editor_exit", "tile_next", "tile_prev", "toggle_pass", "obj_next", "obj_prev", "tile_copy", "tile_paste", "obj_copy", "obj_paste", "tile_check", "tile_find", "obj_check", "obj_find", "toggle_pass", "editor_save", "toggle_highlight_pass"}
	...
	cspells(player)
	for i = 1, #add do player:addSpell(add[i].."") end
...
quit = function(player)
	...
	if player:hasDuration("toggle_highlight_pass") then player:setDuration("toggle_highlight_pass", 0) end
...
option = function(player, type, num)
	...
	return elseif type == "next" then
		if obj+1 <= 19542 then setObject(m,x,y, obj+1) end
		player:talkSelf(0, "<Next> Object : "..obj)
	elseif type == "previous" then
		if obj-1 >= 0 then setObject(m,x,y, obj-1) end
...
cast = function(player)
	...
	if (not input) then anim(player, "Number only!") return else
		if input > 47919 then anim(player, "Max tile: 47919") return else
...
obj_next = {cast = function(player) object.option(player, "next")end}
obj_prev = {cast = function(player) object.option(player, "previous")end}
obj_check = {cast = function(player) object.option(player, "check")end}
obj_copy = {cast = function(player) object.option(player, "copy") end}
obj_paste = {cast = function(player) object.option(player, "paste") end}
...
cast = function(player)
	...
	if (not input) then anim(player, "Number only!") return else
		if input > 19542 then anim(player, "Max Object: 19542") return else
...
cast = function(player)
	...
	if (not input) then anim(player, "Number only!") return else
		if input - 1 >= 0 then
			...
			if getPass(m,x,y) == 0 then id = 235 else id = 228 end
