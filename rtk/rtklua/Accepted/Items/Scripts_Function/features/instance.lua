doInstanceSpawns = function(player)
	doStrengthTrialSpawns(player)
	doSumoCourseSpawns(player)
	doRuinsSpawns(player)
	doCanidaeSpawns(player)
end

doStrengthTrialSpawns = function(player)
	if (player.mapTitle == "Trial of Strength and Wits") then
		strength_trial_spawns.room1(player)
	end

end

doSumoCourseSpawns = function(player)
	if (player.mapTitle == "Sumo Course") then
		sumo_course_spawns.room1(player)
	end
