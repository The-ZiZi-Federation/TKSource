	...
	case 0x03:
		...
		if (maintenance_mode()) {
			if (maintenance_override(RFIFOP(fd, 6), RFIFOB(fd, 5)) == 0) {
				clif_message(fd, 0x03, "Server is undergoing maintenance. Please try again later.");
				break;
			}
		}
		...
		if (require_reg) {
			if (!reg_check(RFIFOP(fd, 6), RFIFOB(fd, 5))) {
				...
				clif_message(fd, 0x03, "You must attach your character to an account to play.\n\nPlease contact administrators to attach your character to an account. If you have issues, please visit our Discord.");
				break;
			}
		}
