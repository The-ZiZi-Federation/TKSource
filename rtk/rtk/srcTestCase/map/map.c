...
void do_term(void) {
	...
	printf("R.O.S.TK Map Server Shutdown.\n");
	add_log("Shutdown.\n");
}
...
int do_init(int argc, char** argv) {
	...
	add_log("");
	add_log("R.O.S.TK Map Server Started.\n");
	...
	printf("R.O.S.TK Map Server is \033[1;32mready\033[0m! Listening at %d.\n", map_port);
...
int map_reset_timer(int v1, int v2) {
	...
	if (reset <= 60000) { //Less than a minute remaining(gonna mass spell everyone)
		if (diff >= 10000) { // every 10 seconds
			sprintf(msg, "R.O.S.TK! Reset in %d seconds", reset / 1000);
			//clif_broadcast("---------------------------------------------------",-1);
		...
		if (diff >= 300000) { // every 5 mins
			sprintf(msg, "R.O.S.TK! Reset in %d minutes", reset / 60000);
			//clif_broadcast("---------------------------------------------------",-1);
			clif_broadcast(msg, -1);
			//clif_broadcast("---------------------------------------------------",-1);
			diff = 0;
		...
		if (diff >= 3600000) { // once every hour
			sprintf(msg, "R.O.S.TK! Reset in %d hours", reset / 3600000);
			//clif_broadcast("---------------------------------------------------",-1);
			clif_broadcast(msg, -1);
			//clif_broadcast("---------------------------------------------------",-1);
			diff = 0;
